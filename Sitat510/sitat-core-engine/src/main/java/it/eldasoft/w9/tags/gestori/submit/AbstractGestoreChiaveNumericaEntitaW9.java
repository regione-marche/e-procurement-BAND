...
public abstract class AbstractGestoreChiaveNumericaEntitaW9 extends AbstractGestoreChiaveNumerica {
	...
	public void afterInsertEntita(TransactionStatus status, DataColumnContainer impl) throws GestoreException {
		...
		case CostantiW9.APERTURA_CANTIERE:
			...
			break;
		default:
			num = new Long(1); 					// caso di default 
		...
		if (insert) {
			if (StringUtils.isNotEmpty(idSchedaLocale)) {
				sqlInstruction = "INSERT INTO W9FASI (CODGARA,CODLOTT,FASE_ESECUZIONE,NUM,DAEXPORT,ID_SCHEDA_LOCALE) VALUES (?,?,?,?,'1','" + idSchedaLocale + "')";
			} else {
				sqlInstruction = "INSERT INTO W9FASI (CODGARA,CODLOTT,FASE_ESECUZIONE,NUM,DAEXPORT) VALUES (?,?,?,?,'1')";
			}
		} else {
			sqlInstruction = "UPDATE W9FASI SET DAEXPORT='1' WHERE  CODGARA = ? AND CODLOTT = ? AND FASE_ESECUZIONE = ? AND NUM = ?";
		}
	
		try {
			this.sqlManager.update(sqlInstruction, new Object[] { codiceGara, codiceLotto, faseEsecuzione, num });
		} catch (SQLException sqlEx) {
			logger.error("Errore nel salvataggio della fase (Fase_esecuzione=" + faseEsecuzione + ", Progressivo=" + num +")", sqlEx);
			throw new GestoreException("Errore nel salvataggio della fase (Fase_esecuzione=" + faseEsecuzione + ", Progressivo=" + num +")", null, sqlEx);
		}
	...
	public void afterUpdateEntita(TransactionStatus status, DataColumnContainer impl) throws GestoreException {
		...
		try {
			this.sqlManager.update("UPDATE W9FASI SET DAEXPORT='1' WHERE  CODGARA = ? AND CODLOTT = ? AND FASE_ESECUZIONE = ? AND NUM = ?",
					new Object[] { codiceGara, codiceLotto, faseEsecuzione, num });
		} catch (SQLException sqlEx) {
