...
public class AperturaCantiereHandler extends AbstractRequestHandler {
  ...
  protected void managePrimoInvio(XmlObject documento,
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di fase scheda cantiere di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicatari(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di fase scheda cantiere di un lotto senza imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (this.existsFase(fase.getW3FACIG(), CostantiW9.APERTURA_CANTIERE)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di una scheda cantiere gia' presente sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (!ignoreWarnings) {
      ...
      if (!isR) {
        ...
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Primo invio di fase scheda cantiere di un lotto che non prevede manodopera");
        // si termina con questo errore
        return;
      }
  ...
  protected void manageRettifica(XmlObject documento,
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non appaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di fase scheda cantiere di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicatari(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto senza imprese aggiudicatarie sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di fase scheda cantiere di un lotto senza imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsFase(fase.getW3FACIG(), CostantiW9.APERTURA_CANTIERE)) {
      logger.error("La richiesta di rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di una scheda cantiere non presente sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (!ignoreWarnings) {
      ...
      if (!isR) {
        ...
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
            "Rettifica di fase scheda cantiere di un lotto che non prevede manodopera");
        // si termina con questo errore
        return;
      }
    }
