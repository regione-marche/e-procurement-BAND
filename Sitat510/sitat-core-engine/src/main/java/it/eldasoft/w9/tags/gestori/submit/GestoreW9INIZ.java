    ...
    if (impl.isColumn("W9SIC.CODGARA")) {
      try {
        if (impl.getLong("W9SIC.CODGARA") == null) {
          ...
          impl.setValue("W9SIC.NUM_SIC", impl.getLong("W9INIZ.NUM_INIZ"));
          impl.insert("W9SIC", this.getSqlManager());
        } else {
    ...
    if (impl.isColumn("W9MISSIC.CODGARA")) {
      try {
        if (impl.getLong("W9MISSIC.CODGARA") == null) {
          ...
          impl.setValue("W9MISSIC.NUM_MISS", impl.getLong("W9INIZ.NUM_INIZ"));
          impl.getColumn("W9MISSIC.CODGARA").setChiave(true);
          impl.getColumn("W9MISSIC.CODLOTT").setChiave(true);
          impl.getColumn("W9MISSIC.NUM_MISS").setChiave(true);
          impl.insert("W9MISSIC", this.getSqlManager());
        } else {
  ...
  public void preInsert(TransactionStatus status, DataColumnContainer impl) throws GestoreException {
    ...
    try {
      if (impl.isColumn("W9INIZ.NUM_INIZ")) {
        this.sqlManager.update("insert into W9SIC (CODGARA, CODLOTT, NUM_SIC) values (?,?,?)", 
            new Object[]{impl.getLong("W9INIZ.CODGARA"), impl.getLong("W9INIZ.CODLOTT"), new Long(1) });

        if (! this.geneManager.getProfili().checkProtec(
            (String) this.getRequest().getSession().getAttribute(CostantiGenerali.PROFILO_ATTIVO), "PAGE", "VIS", "W9.W9INIZ-scheda.SICUREZZA")) {
          this.sqlManager.update("UPDATE W9SIC set PIANSCIC=2, DIROPE=2, TUTOR=2 where CODGARA=? and CODLOTT=? and NUM_SIC=1",
              new Object[]{ impl.getLong("W9INIZ.CODGARA"), impl.getLong("W9INIZ.CODLOTT")} );
        }
      }
    } catch (SQLException e) {
        throw new GestoreException(
