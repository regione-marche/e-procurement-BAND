...
public abstract class AbstractRequestHandler {
  ...
  /**
   * Verifica se la gara indicata a partire dal CIG fornito nel tracciato
   * e' una gara per intervento di ricostruzione alluvione Lunigiana e Elba
   * (W9GARA.RIC_ALLUV = '1').
   * 
   * @param fase fase del flusso
   * @return Ritorna true se la gara e' per intervento di ricostruzione alluvione Lunigiana e Elba
   * @throws SQLException SQLException
   */
  ...
  /**
   * Verfica se il lotto e' stato aggiudicato ossia se nella fase "Esito"
   * il lotto e' indicato come aggiudicato (W9ESITO.ESITO_PROCEDURA = 1).
   * 
   * @param fase elemento fase del flusso
   * @return  Restituisce TRUE se il lotto e' aggiudicato ossia se nella fase "Esito"
   *          il lotto e' indicato come aggiudicato (W9ESITO.ESITO_PROCEDURA = 1)
   * @throws SQLException SQLException
   */
  ...
  protected final boolean existsAppalto(final FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        ...
        + "and w9appa.num_appa = 1 and w9lott.cig = ?",
  ...
  protected final boolean existsAggiudicazione(final FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        "select count(w9aggi.codgara) from w9aggi, w9lott "
        + "where w9aggi.codgara = w9lott.codgara and w9aggi.codlott = w9lott.codlott "
        + "and w9aggi.num_appa = 1 and w9lott.cig = ?",
        new Object[] { fase.getW3FACIG() });
    return numeroOccorrenze.intValue() > 0;
  }
  ...
  protected boolean existsIniz(FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        ...
        + "w9iniz.num_iniz = 1 and w9lott.cig = ?",
        new Object[] { fase.getW3FACIG() });
  ...
  /**
   * Verifica se esiste in banca dati la fase passata come argomento per il CIG specificato
   ...
   * @throws GestoreException GestoreException
   */
  protected boolean existsFase(final String cig, final int faseEsecuzione)
      throws SQLException, GestoreException {
    return UtilitySITAT.existsFase(cig, faseEsecuzione, sqlManager);
  }
   ...
   * @param cig Codice CIG
   * @param faseEsecuzione Fase esecuzione
   * @param numeroProgressivo numeroProgressivo
   * @return Ritorna true se la fase esiste, false altrimenti
   * @throws SQLException SQLException
   */
  protected boolean existsFase(final String cig, final int faseEsecuzione,
      final int numeroProgressivo) throws SQLException {
    return UtilitySITAT.existsFase(cig, faseEsecuzione, new Long(numeroProgressivo), sqlManager);
  }
