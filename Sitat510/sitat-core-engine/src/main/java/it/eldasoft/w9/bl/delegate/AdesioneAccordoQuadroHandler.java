...
public class AdesioneAccordoQuadroHandler extends AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di primo invio ha un cig (" + fase.getW3FACIG()
          + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di adesione accordo quadro di un lotto non presente in banca dati");
      continua = false;
    }
    ...
    if (continua && this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di adesione accordo quadro di un lotto gia' appaltato in banca dati");
      continua = false;
    }
    ...
    if (continua && !ignoreWarnings) {
      ...
      if ( !(isAAQ)) {
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
            "Primo invio di una adesione accordo quadro non prevista");
  ...
  private void manageRettificaOIntegrazione(XmlObject documento, DataColumnContainer datiAggiornamento,
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di " + this.getDescrioneTipoInvio(tipoInvio) +
          " di adesione accordo quadro ha un cig (" + fase.getW3FACIG() + ") di un lotto non appaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          UtilityStringhe.capitalize(this.getDescrioneTipoInvio(tipoInvio)) +
      " di adesione accordo quadro di un lotto non appaltato nella banca dati");
      continua = false;
    }
    ...
    if (continua && !ignoreWarnings) {
      ...
      if ( !(isAAQ)) {
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
            UtilityStringhe.capitalize(this.getDescrioneTipoInvio(tipoInvio))
            + " di una adesione accordo quadro non prevista");
        
        if (!isAAQ) {
          logger.error("La richiesta di " + this.getDescrioneTipoInvio(tipoInvio) +
            " di adesione accordo quadro e' relativo ad una gara che " +
            "non e' una adesione ad accordo quadro senza confronto competitivo (TIPO_APP <> 11)");
        }
        continua = false;
      }
    ...
    if (continua) {
      this.deleteDatiFlusso(doc.getRichiestaRichiestaRispostaSincronaAdesioneAccordoQuadro().getFase());
  ...
  private void deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9appa where codgara=? and codlott=?", new Object[]{codGara, codLott }, 1);
    this.sqlManager.update(
        "delete from w9aggi where codgara=? and codlott=?", new Object[]{codGara, codLott });
    this.sqlManager.update(
        "delete from w9inca where codgara=? and codlott=? and sezione=?",
        new Object[] {codGara, codLott, "RS"});
    this.sqlManager.update(
        "delete from w9fina where codgara=? and codlott=?", new Object[]{codGara, codLott});
  }
