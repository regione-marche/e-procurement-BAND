...
AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di subappalto di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicatari(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di subappalto di un lotto senza imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (this.existsFase(fase.getW3FACIG(), CostantiW9.SUBAPPALTO, fase.getW3NUM5())) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto gia' subappaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di subappalto di un lotto gia' subappaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if ((!isS2 && !isAII) || isE1 || !isOrd) {
        ...
        if (!(isS2)) {
          logger.error("La richiesta di primo invio subappalto fa riferimento ad un lotto non superiore a 40000"
              + " (cig=" + fase.getW3FACIG() + ")");
        }
        if (isAII) {
          logger.error("La richiesta di primo invio subappalto fa riferimento ad un lotto (cig="
              + fase.getW3FACIG() + ") che riferisce ad un intervento che non e' di ricostruzione "
              + "da alluvione Lunigiana Elba");
        }
        if (isE1) {
          logger.error("La richiesta di primo invio subappalto fa riferimento ad un lotto escluso"
              + " (cig=" + fase.getW3FACIG() + ")");
        }
        if (!isOrd) {
          logger.error("La richiesta di primo invio subappalto fa riferimento ad un lotto senza settori oridnari"
              + " (cig=" + fase.getW3FACIG() + ")");
        }
        // si termina con questo errore
        return;
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non appaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di subappalto di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicatari(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto senza imprese aggiudicatarie sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di subappalto di un lotto senza imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsFase(fase.getW3FACIG(), CostantiW9.SUBAPPALTO, fase.getW3NUM5())) {
      logger.error("La richiesta di rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto non subappaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (!ignoreWarnings) {
      ...
      if (!isS2 && !isAII) {
        logger.error("La richiesta di rettifica di un subappalto non previsto per il lotto con CIG=" + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Rettifica di variante non prevista");
        if (!(isS2)) {
          logger.error("La richiesta di integrazione subappalto fa riferimento ad un lotto non superiore a 40000"
              + " (cig=" + fase.getW3FACIG() + ")");
        }
        if (isAII) {
          logger.error("La richiesta di integrazione subappalto fa riferimento ad un lotto (cig="
              + fase.getW3FACIG() + ") che riferisce ad un intervento che non e' di ricostruzione "
              + "da alluvione Lunigiana Elba");
        }
        if (isE1) {
