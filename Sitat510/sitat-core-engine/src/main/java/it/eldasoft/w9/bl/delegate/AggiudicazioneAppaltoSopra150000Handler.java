...
public class AggiudicazioneAppaltoSopra150000Handler extends AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (this.existsAppalto(fase)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto gia' appaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di aggiudicazione appalto di un lotto gia' appaltato in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di aggiudicazione appalto semplificata di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    if (!ignoreWarnings) {
      ...
      if (!isS2 || isE1) {
        logger.error("La richiesta di rettifica di aggiudicazione non prevista per il lotto con CIG=" + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Rettifica di aggiudicazione non prevista");
        if (!isS2) {
  ...
  throws SQLException, GestoreException {
    ...
    if (tab18.isSetW9APMODRIAG()) {
      dccAppalto.addColumn("W9APPA.MODALITA_RIAGGIUDICAZIONE", JdbcParametro.TIPO_TESTO,
          tab18.getW9APMODRIAG().toString());
    }
    ...
    if (doc.getRichiestaRichiestaRispostaSincronaAggiudicazioneAppaltoSopra150000().getListaTab182().sizeOfTab182Array() > 0) {
      ...
      Long numeroIncarichi = (Long) this.sqlManager.getObject(
          "select count(*) from w9inca where codgara = ? and codlott = ? and sezione = ?",
          new Object[] { codGara, codLott, "RA" });
      
      if (numeroIncarichi != null && numeroIncarichi > 0) {
        this.sqlManager.update(
            "delete from w9inca where codgara = ? and codlott = ? and (sezione = ? or sezione = ?)",
                new Object[] { codGara, codLott, "PA", "RA" });
      }
    }
  ...
  private void manageIntegrazioneORettifica(
      ...
      SQLException {
    this.deleteDatiFlusso(doc.getRichiestaRichiestaRispostaSincronaAggiudicazioneAppaltoSopra150000().getFase());
  ...
  private void deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9appa where codgara = ? and codlott = ?", new Object[] {
            codGara, codLott }, 1);
    this.sqlManager.update(
        "delete from w9aggi where codgara = ? and codlott = ?", new Object[] {
            codGara, codLott });
    this.sqlManager.update(
        "delete from w9inca where codgara = ? and codlott = ? and (sezione = ? or sezione = ?)",
        new Object[] { codGara, codLott, "PA", "RA" });
    this.sqlManager.update(
        "delete from w9fina where codgara = ? and codlott = ?", new Object[] {
            codGara, codLott });
  }
