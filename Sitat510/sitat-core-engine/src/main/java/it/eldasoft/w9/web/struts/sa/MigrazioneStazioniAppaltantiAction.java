...
public class MigrazioneStazioniAppaltantiAction extends DispatchActionBaseNoOpzioni {
  ...
  public ActionForward inizializza(ActionMapping mapping, ActionForm form,
    ...
    try {
      ...
      mb.setNomeStazAppOrigine((String)
          this.sqlManager.getObject("select NOMEIN from UFFINT where CODEIN=?", new Object[] { mb.getCodiceStazAppOrigine() } ));
  ...
  public ActionForward controlliPreliminari(ActionMapping mapping, ActionForm form,
    ...
    if (mb != null) {
      try {
        HashMap<String, Object> hm = this.migrazioneSaManager.controlloDatiGaraSABean(mb,
            mb.getCodiceFiscaleStazAppDestinazione(), (long) profiloUtente.getId());
        
        if (!((Boolean) hm.get("esito")).booleanValue()) {
          if (! hm.containsKey("msgErr")) {
          ...
          mb.setNomeStazAppDestinazione((String) this.sqlManager.getObject(
              ...
              new Object[] { mb.getGaraSABeanInElaborazione().getCfsaDestinazione() }));
