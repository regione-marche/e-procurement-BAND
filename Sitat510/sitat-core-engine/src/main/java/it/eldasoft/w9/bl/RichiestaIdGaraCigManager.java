...
public class RichiestaIdGaraCigManager {
  ...
  public HashMap<String, Object> consultaLotto(final String codiceCIG, final String idAvGara,
    ...
    case CostantiSimog.SA_CONSULTA_GARA_ACCESSO_VIA_WS:
    case CostantiSimog.SA_CONSULTA_GARA_ACCESSO:
      this.consultaLottoAccessoDiretto(codiceCIG, idAvGara, codUffInt, syscon, resultMap, simoguser, simogpass);
      break;
          ...
          if (consultaNumeroGaraResponseDoc.getConsultaNumeroGaraResponse().isSetReturn()) {
            if (consultaNumeroGaraResponseDoc.getConsultaNumeroGaraResponse().getReturn().getSuccess()) {
              if (consultaNumeroGaraResponseDoc.getConsultaNumeroGaraResponse().getReturn().isSetSchedaGaraCig()) {
                Stack<String> listaCig = new Stack<String>();
                ...
                for (int ir=0; ir < arrayCig.length; ir++) {
                  listaCig.push(arrayCig[ir]);
                }
                consultaGaraLottiBean.setListaCig(listaCig);
            ...
            if (consultaGaraResponseDoc.getConsultaGaraResponse().isSetReturn()) {
              ...
              if (responseConsultaGara.getSuccess()) {
                ...
                consultaLottoBean = this.consultaGaraUnificatoManager.consultaGaraUnificatoWS(
                    codiceCIG, idAvGara, codUffInt, syscon, schedaType, garaType, lottoType);
              } else {
   ...
   * @throws SQLException
   */
  private void consultaGaraAccessoIndiretto(final String idAvGara,
      final String codUffInt, final int syscon, HashMap<String, Object> resultMap, ConsultaGaraLottiBean consultaGaraLottiBean) 
      ...
      try {
        ...
        if (responseConsultaGara.isSuccess() && StringUtils.isNotEmpty(responseConsultaGara.getGaraXML())) {
          ...
          if (consultaNumeroGaraResponseDocument.getConsultaNumeroGaraResponse() != null && 
            ...
            if (schedaGaraCig.getCIGArray() != null && schedaGaraCig.getCIGArray().length > 0) {
              Stack<String> listaCig = new Stack<String>();
    ...
    if (StringUtils.isNotEmpty(urlWsORT)) {
      ...
      try {
        ...
        ConsultaLottoBean consultaLottoBean = null;
          ...
          logger.info("Invocazione del metodo getGaraXML del WsOsservatorio (IdGara="
              + idAvGara + ", CIG=" + codiceCIG + ", cfrup=" + cfrup + ", cfsa=" + cfsa + ")");
          responseConsultaGara = wsOsservatorioProxy.getGaraXML(codiceCIG, idAvGara, cfrup, cfsa, true);
          logger.info("Invocato il metodo getGaraXML del WsOsservatorio (IdGara="
              + idAvGara + ", CIG=" + codiceCIG + ", cfrup=" + cfrup + ", cfsa=" + cfsa + ")");
          ...
          if (responseConsultaGara.isSuccess()) {
            ...
            consultaLottoBean = this.consultaGaraUnificatoManager.consultaGaraUnificatoPDD(codiceCIG, 
              idAvGara, codUffInt, syscon, consultaGaraResponseDocument.getConsultaGaraResponse().getReturn().getGaraXML(),
   ...
   * @throws SQLException
   */
  private HashMap<String, Object> consultaGaraAccessoIndirettoPddXmlSimog(final String codiceCIG,
  ...
  private HashMap<String, Object> consultaGaraAccessoIndirettoPddCigGaraSimog(final String idAvGara,
    ...
    if (StringUtils.isNotEmpty(urlWsORT)) {
      ...
      try {
        ...
        logger.error("Errore nell'interazione con il WS Osservatorio per la chiamata consultaGara "
            + "durante la funzione 'Carica lotto da SIMOG' con idAvGara='" + idAvGara
            + " da parte dell'utente syscon = " + syscon 
            + " per conto della stazione appaltante con codUffInt='" + codUffInt + "'", re);
      ...
      /*} catch (ServiceException se) {
        ...
        logger.error("Errore inaspettato nel collegamento al WS Osservatorio per la chiamata "
            + "consultaGara durante la funzione 'Carica lotto da SIMOG' con idAvGara='"
  ...
  private synchronized void riallineaDatiSimogAccessoDiretto(final String codiceCIG,
    ...
    if (StringUtils.isNotEmpty(urlWsSimogPDD)
      ...
      try {
        // Creazione dell'oggetto Authenticator contenente i parametri necessari alla basic authentication
        HttpTransportProperties.Authenticator basicAuth = this.ticketSimogManager.getBasicAuthentication();
        ...
        if (basicAuth == null) {
            ...
            // attraverso la porta di PDD, la quale richiede che l'url sia seguita dal metodo invocato
        	  simogWSOssStub = new SimogWSPDDServiceStub(urlWsSimogPDD + "/login");
        	  simogWSOssStub._getServiceClient().getOptions().setProperty(
        ...
        if (loginResponse.isSetReturn()) {
          ...
          if (responseCheckLogin.getSuccess()) {
            ...
            if (basicAuth != null) {
            	simogWSOssStub = new SimogWSPDDServiceStub(urlWsSimogPDD + "/consultaGara");
                simogWSOssStub._getServiceClient().getOptions().setProperty(
                  HTTPConstants.AUTHENTICATE, basicAuth);
            }
            ...
            if (basicAuth == null) {
              ...
              simogWSOssStub = new SimogWSPDDServiceStub(urlWsSimogPDD + "/chiudiSessione");
              simogWSOssStub._getServiceClient().getOptions().setProperty(
                  HTTPConstants.AUTHENTICATE, basicAuth);
            }
  ...
  private synchronized void riallineaDatiSimogUnificatoPDD(
    ...
    /*Long codLott = (Long) this.sqlManager.getObject(
        "select max(CODLOTT)+1 from W9LOTT where CODGARA=?",
        new Object[]{codGara});
    if (codLott == null) {
      codLott = new Long(1);
    }*/
    ...
    if (lottoType.isSetFLAGESCLUSO() &&
          it.avlp.simog.massload.xmlbeans.FlagSNType.S.toString().equals(
              lottoType.getFLAGESCLUSO())) {
      listaSqlLottoParams.add("1");
    } else {
   ...
   * - se GaraType.MODO_REALIZZAZIONE<>9 e nella base dati e' verificata la condizione: isSAQ
   *   ed esiste la fase A20  
   * - se GaraType.MODO_REALIZZAZIONE=9 e nella base dati e' verificata la condizione: not isSAQ
   *   ed esistono le fasi A06 o A07
  ...
  private boolean isGaraLottoSimogCompatibile(final String codiceCIG, BigDecimal importoLottoInSimog,
    ...
    boolean isE1 = UtilitySITAT.isE1(codiceCIG, this.sqlManager);
    boolean isSAQ = UtilitySITAT.isSAQ(codiceCIG, this.sqlManager);
    boolean isAAQ = UtilitySITAT.isAAQ(codiceCIG, this.sqlManager);
 
    boolean esisteA05 = UtilitySITAT.existsFase(codiceCIG, CostantiW9.AGGIUDICAZIONE_SOPRA_SOGLIA,
        this.sqlManager);
    boolean esisteA06 = UtilitySITAT.existsFase(codiceCIG, CostantiW9.INIZIO_CONTRATTO_SOPRA_SOGLIA,
        this.sqlManager);
    boolean esisteA04 = UtilitySITAT.existsFase(codiceCIG, CostantiW9.FASE_SEMPLIFICATA_AGGIUDICAZIONE,
        this.sqlManager);
    boolean esisteA07 = UtilitySITAT.existsFase(codiceCIG, CostantiW9.FASE_SEMPLIFICATA_INIZIO_CONTRATTO,
        this.sqlManager);
    boolean esisteA20 = UtilitySITAT.existsFase(codiceCIG, CostantiW9.STIPULA_ACCORDO_QUADRO,
        this.sqlManager);
    boolean esisteA21 = UtilitySITAT.existsFase(codiceCIG, CostantiW9.ADESIONE_ACCORDO_QUADRO,
        this.sqlManager);
    ...
    if (result) {
      if (!"9".equals(modoRealizzazioneGaraInSimog)
          && isSAQ
          && esisteA20) {
        result = false;
        
        logger.info("isGaraLottoSimogCompatibile: il lotto con CIG=" + codiceCIG + " e la relativa gara non verificano la seguente condizione: "
            + "(GaraType.MODO_REALIZZAZIONE<>9) and isSAQ and esisteFaseA20");
      }
    }
    ...
    if (result) {
      if ("9".equals(modoRealizzazioneGaraInSimog)
        ...
        logger.info("isGaraLottoSimogCompatibile: il lotto con CIG=" + codiceCIG + " e la relativa gara non verificano la seguente condizione: "
            + "(GaraType.MODO_REALIZZAZIONE=9) and NOT isSAQ and (esisteFaseA06 or esisteFaseA07)");
      }
    }
    
    if (result) {
      if (!"11".equals(modoRealizzazioneGaraInSimog)
          && isAAQ
          && esisteA21) {
        result = false;
        
        logger.info("isGaraLottoSimogCompatibile: il lotto con CIG=" + codiceCIG + " e la relativa gara non verificano la seguente condizione: "
            + "(GaraType.MODO_REALIZZAZIONE<>11) and isAAQ and esisteFaseA21");
      }
    }
    ...
    if (result) {
      if ("11".equals(modoRealizzazioneGaraInSimog)
        ...
        logger.info("isGaraLottoSimogCompatibile: il lotto con CIG=" + codiceCIG + " e la relativa gara non verificano la seguente condizione: "
            + "(GaraType.MODO_REALIZZAZIONE=11) and NOT isAAQ and (esisteFaseA04 or esisteFaseA05)");
      }
    }
   ...
   */
  public HashMap<String, Object> getXmlSimog(final String codiceCIG, long syscon, String codUffint) {
    ...
    switch (tipoAccessoWSSimog) {
    case CostantiSimog.SA_CONSULTA_GARA_ACCESSO_INDIRETTO_VIA_WS:
      hm = this.consultaGaraAccessoIndirettoPddXmlSimog(codiceCIG, syscon, codUffint);
      break;
