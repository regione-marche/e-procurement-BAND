...
public class StipulaAccordoQuadroHandler extends AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (continua && !super.existsAppalto(fase)) {
      continua = false;
      logger.error("La richiesta di primo invio non ha un appalto associato al lotto "
          + "presente in DB (cig = " + fase.getW3FACIG() + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di una stipula accordo quadro con appalto non esistente in banca dati");
    }

    if (continua && !super.existsAggiudicatari(fase)) {
      continua = false;
      logger.error("La richiesta di primo invio non ha una aggiudicazione associata al lotto "
          + "presente in DB (cig = " + fase.getW3FACIG() + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di una Stipula accordo quadro con aggiudicazione non esistente in banca dati");
    }

    if (continua && existsIniz(fase)) {
      continua = false;
      logger.error("La richiesta di primo invio di una stipula accordo quadro con fase iniziale "
          + "non presente nel DB (cig = " + fase.getW3FACIG() + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (continua && !ignoreWarnings) {
      ...
      if (! isSAQ) {
        ...
        if (!isSAQ) {
          logger.error("Richiesta di primo invio di una stipula accordo quadro relativa ad un "
            + "lotto che non corrisponde ad una stipula accordo quadro (TIPO_APP <> 9) (cig = "
            + fase.getW3FACIG() + ")");
        }
      }
    }
  ...
  private void manageRettificaOIntegrazione(XmlObject documento, DataColumnContainer datiAggiornamento,
    ...
    if (!super.existsLotto(fase)) {
      continua = false;
      logger.error("La richiesta di " + this.getDescrioneTipoInvio(tipoInvio) + " ha un cig ("
          + fase.getW3FACIG() + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (continua && !super.existsAppalto(fase)) {
      ...
      logger.error("La richiesta di " + this.getDescrioneTipoInvio(tipoInvio)
          + " non ha un appalto associato al lotto presente in DB (cig = " + fase.getW3FACIG() + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          UtilityStringhe.capitalize(this.getDescrioneTipoInvio(tipoInvio))
          + " di una stipula accordo quadro con appalto non esistente in banca dati");
    }

    if (continua && !super.existsAggiudicatari(fase)) {
      continua = false;
      logger.error("La richiesta di " + this.getDescrioneTipoInvio(tipoInvio)
          + " non ha una aggiudicazione associata al lotto presente in DB (cig = " + fase.getW3FACIG() + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          UtilityStringhe.capitalize(this.getDescrioneTipoInvio(tipoInvio))
          + " di una Stipula accordo quadro con aggiudicazione non esistente in banca dati");
    }

    if (continua && !existsIniz(fase)) {
      continua = false;
      logger.error("La richiesta di " + this.getDescrioneTipoInvio(tipoInvio)
          + " di una stipula accordo quadro con fase iniziale non presente nel DB(cig = "
          + fase.getW3FACIG() + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          UtilityStringhe.capitalize(this.getDescrioneTipoInvio(tipoInvio))
          + " di una stipula accordo quadro gia' esistente in banca dati");
    }
    ...
    if (continua && !ignoreWarnings) {
      ...
      if (!(isSAQ)) {
        ...
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
            UtilityStringhe.capitalize(this.getDescrioneTipoInvio(tipoInvio))
            + " di una stipula accordo quadro non prevista");
        if (!isSAQ) {
          logger.error("Richiesta di " + this.getDescrioneTipoInvio(tipoInvio)
              + " di una stipula accordo quadro relativa ad un lotto che non corrisponde ad una "
              + "stipula accordo quadro (TIPO_APP <> 9) (cig = " + fase.getW3FACIG() + ")");
        }
      }
    }
  ...
  private void deleteDatiFlusso(final FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9iniz where codgara = ? and codlott = ?", new Object[] {
            codGara, codLott }, 1);
    this.sqlManager.update(
        "delete from w9pues where codgara = ? and codlott = ?", new Object[] {
            codGara, codLott });
  }
