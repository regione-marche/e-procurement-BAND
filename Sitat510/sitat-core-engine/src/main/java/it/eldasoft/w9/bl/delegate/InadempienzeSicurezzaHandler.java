...
public class InadempienzeSicurezzaHandler extends AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      ...
      "Primo invio di Inadempienze con lotto non esistente in banca dati");
      return;
    }
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di Inadempienze con un lotto non appaltato in banca dati");
    ...
    if (!this.existsAggiudicazione(fase)) {
      logger.error("La richiesta di primo invio ha un cig ("
          ...
          + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di Inadempienze con  un lotto non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (this.existsFase(fase.getW3FACIG(), CostantiW9.INADEMPIENZE_SICUREZZA_REGOLARITA_LAVORI, fase.getW3NUM5())) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto che presenta gia' inadempienze in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di inadempienze gia' presenti in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if (!isLottoConManodopera) {
        ...
        "Primo invio di scheda inadempienze di un lotto che non prevede manodopera");
        // si termina con questo errore
        return;
      }
    }
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non appaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di Inadempienze con un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicazione(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di Inadempienze con un lotto non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if (!isLottoConManodopera) {
        logger.error("La richiesta di rettifica ha un lotto che non prevede manodopera");
        this.setEsito(datiAggiornamento,
            StatoComunicazione.STATO_WARNING.getStato(),
        "Rettifica di scheda inadempienze di un lotto che non prevede manodopera");
        // si termina con questo errore
        return;
      }
    }
