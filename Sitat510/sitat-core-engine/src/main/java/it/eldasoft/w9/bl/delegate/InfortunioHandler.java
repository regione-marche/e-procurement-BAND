...
public class InfortunioHandler extends AbstractRequestHandler {
	  ...
	  if (!ignoreWarnings) {
	    ...
	    if (!isLottoConManodopera) {
	      ...
	      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
	          "Primo invio di richiesta infortunio di un lotto che non prevede manodopera");
	      // si termina con questo errore
	      return;
	    }
    }
  ...
  protected void manageRettifica(XmlObject documento,
    ...
    // if (!this.existsInfortunio(fase)) {
    if (!this.existsFase(fase.getW3FACIG(),
        CostantiW9.SCHEDA_SEGNALAZIONI_INFORTUNI, fase.getW3NUM5())) {
      logger.error("La richiesta di rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto con infortunio non presente sul DB");
      this.setEsito(
          datiAggiornamento,
          StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di richiesta infortunio di un lotto con infortunio non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if (!isLottoConManodopera) {
        ...
        this.setEsito(datiAggiornamento,
            ...
            "Rettifica di richiesta infortunio di un lotto che non prevede manodopera");
        // si termina con questo errore
        return;
      }
    }
