...
AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di conclusione appalto semplificata di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicazione(fase)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di una aggiudicazione non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di conclusione appalto semplificata di una aggiudicazione non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsInizio(fase)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto senza fase una inziale sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di conclusione appalto semplificata di un lotto senza una fase inziale in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (this.existsConclusione(fase)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto avente gia' una fase conclusione sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di conclusione appalto semplificata di un lotto avente gia'a una fase conclusione in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di conclusione appalto semplificata di un lotto non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non appaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di conclusione appalto semplificata di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicazione(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di una aggiudicazione non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di conclusione appalto semplificata di una aggiudicazione non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsInizio(fase)) {
      ...
      "Rettifica di conclusione appalto semplificata di un lotto senza una fase inziale in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if (!isS2 || isE1 || isSAQ || !isOrd) {
        logger.error("La richiesta di rettifica di conclusione del contratto non previsto per il lotto con CIG=" + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Rettifica di conclusione del contratto non previsto");
        if (!isS2) {
        ...
        if (isSAQ) {
          logger.error("La richiesta di rettifica ha il lotto relativo ad una stipula accordo (TIPO_APP=9)");
        }
        if (!isOrd) {
          logger.error("La richiesta di rettifica ha il lotto senza settori oridnari");
        }
        // si termina con questo errore
        return;
      }
  ...
  throws SQLException, GestoreException {
    ...
    // S.C. 16/03/2011 Gli attributi W3NUMINFO, W3NUMMORT e W3NUMPERM sono diventati obbligatori
  ...
  private void deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9conc where codgara = ? and codlott = ?",
