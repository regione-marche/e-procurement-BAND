...
AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAggiudicazione(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di una Fase iniziale di esecuzione del contratto di un lotto non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (this.existsInizio(fase)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto che possiede gia' fase una inziale sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di una Fase iniziale di esecuzione del contratto di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicazione(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di una Fase iniziale di esecuzione del contratto di un lotto non presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if ((!isE1 && isS2 && isOrd) || isSAQ) {
        logger.error("La richiesta di rettifica di Fase iniziale di esecuzione semplificata del contratto non previsto per il lotto con CIG=" + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Primo invio di inizio esecuzione semplificata del contratto non previsto");
        logger.error("La richiesta di rettifica ha un lotto con importo maggiore o uguale a 40000 e non escluso e per settori speciali, oppure e' una stipula accordo quadro");
        return;
      }
    }
  ...
  private void deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9sic where codgara = ? and codlott = ? and num_sic = ?",
