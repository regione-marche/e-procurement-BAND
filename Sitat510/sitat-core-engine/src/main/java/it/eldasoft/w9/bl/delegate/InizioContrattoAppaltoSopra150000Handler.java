...
public class InizioContrattoAppaltoSopra150000Handler extends AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAggiudicatari(fase)) {
      logger.error("La richiesta di primo invio non ha una aggiudicazione con questi codGara e codLott ("
          + codGara + " e " + codLott + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di una Fase iniziale di esecuzione del contratto con aggiudicazione non esistente in banca dati");
      return;
    }

    if (this.existsIniz(fase)) {
      logger.error("La richiesta di primo invio ha un codGara e codLott ("
          + codGara + " e " + codLott
          + ") di una Fase iniziale di esecuzione del contratto gia' esistente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
...
//          return;
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica non ha un appalto con questi codGara e codLott ("
          + codGara + " e " + codLott + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di una Fase iniziale di esecuzione del contratto con appalto non esistente in banca dati");
      return;
    }

    if (!this.existsAggiudicatari(fase)) {
      logger.error("La richiesta di rettifica non ha una aggiudicazione con questi codGara e codLott ("
          + codGara + " e " + codLott + ") ");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di una Fase iniziale di esecuzione del contratto con aggiudicazione non esistente in banca dati");
      return;
    }

    if (!this.existsIniz(fase)) {
      logger.error("La richiesta di rettifica ha un codGara e codLott ("
          + codGara + " e " + codLott
          + ") di una Fase iniziale di esecuzione del contratto non esistente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (!ignoreWarnings) {
      ...
      if (!isS2 || isE1 || !isOrd) {
        logger.error("La richiesta di rettifica di Fase iniziale di esecuzione del contratto non previsto per il lotto con CIG=" + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Rettifica di inizio esecuzione del contratto non previsto");
  ...
  private void deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9iniz where codgara = ? and codlott = ?",
        new Object[] { codGara, codLott }, 1);
    this.sqlManager.update(
        "delete from w9inca where codgara = ? and codlott = ? and sezione= 'IN'",
        new Object[] { codGara, codLott});
    this.sqlManager.update(
        "delete from w9pues where codgara = ? and codlott = ?",
        new Object[] { codGara, codLott });
    this.sqlManager.update(
        "delete from w9inca where codgara = ? and codlott = ? and SEZIONE='IN' ",
        new Object[] { codGara, codLott });
    this.sqlManager.update(
        "delete from w9sic where codgara = ? and codlott = ? ",
        new Object[] { codGara, codLott });
    this.sqlManager.update(
        "delete from w9missic where codgara = ? and codlott = ? and num_miss = ?",
        new Object[] { codGara, codLott, new Long(1) });
    this.sqlManager.update(
        "delete from w9docfase where codgara = ? and codlott = ? and fase_esecuzione = ? and num_fase = ?",
        new Object[] { codGara, codLott, new Long(CostantiW9.MISURE_AGGIUNTIVE_SICUREZZA), new Long(1) });
  }
