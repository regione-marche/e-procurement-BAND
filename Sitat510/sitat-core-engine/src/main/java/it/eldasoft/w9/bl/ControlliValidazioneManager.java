   ...
   *
	...
	public HashMap<String, Object> validate(Object[] params, String custom, String profiloAttivo) throws JspException {
		...
		try {
			...
			if (custom == null || !custom.equals("RM")) {
				...
				} else if (CostantiW9.AREA_GARE_ENTINAZIONALI == areaInvio) {
					// *** Area 5 - Gare enti nazionali e sotto i 40.000 euro
					// ***
					Long codgara = Long.parseLong(params[2].toString());
					this.validazione983(codgara, listaControlli);
				}
			} else {
				if (CostantiW9.AREA_FASI_DI_GARA == areaInvio) {
					// *** Area 1 - Flussi del lotto (fasi/eventi) ***
	...
	private void validazione1(Long codgara, Long codlott, Long num, List<Object> listaControlli)
		...
		try {
			Vector<?> datiW9LOTT = this.sqlManager.getVector(
					"select FLAG_ENTE_SPECIALE, TIPO_CONTRATTO, ID_SCELTA_CONTRAENTE "
			...
			if (datiW9APPA != null && datiW9APPA.size() > 0) {
				...
				/*if (flagEnteSpeciale != null && "S".equals(flagEnteSpeciale) && (idSceltaContraente != null && (!idSceltaContraente.equals(new Long(9))) && (!idSceltaContraente.equals(new Long(10))))) {
		...
		try {
			...
			if (datiW9APPA != null && datiW9APPA.size() > 0) {
				...
				if (valSogliaAnomalia != null) {
					if (offertaMassimo != null && offertaMinima != null && (numOfferteAmmesse != null && numOfferteAmmesse.longValue() >= 5)) {
				  ...
				  if ((idModoGara != null && idModoGara.longValue() == 4) 
				      && (numOfferteAmmesse != null && numOfferteAmmesse.longValue() >= 5)) {
				    listaControlli.add(((new Object[] { "E", pagina, this.getDescrizioneCampo(nomeTabella, "VAL_SOGLIA_ANOMALIA"),
            "Non è stata indicata la soglia di anomalia" })));
				...
				if ((new Long(1).equals(idModoGara) || new Long(4).equals(idModoGara)) &&
					...
					this.addAvviso(listaControlli, nomeTabella, "PERC_RIBASSO_AGG", "E", pagina, "Valorizzare il campo");
				}

				if (new Long(2).equals(idModoGara) && percRibassoAgg == null && percOffAumento == null) {
					this.addAvviso(listaControlli, nomeTabella, "PERC_OFF_AUMENTO", "W", pagina, "Valorizzare il campo");
				}
		...
		try {
			Long conteggiow9inca = (Long) this.sqlManager.getObject("select count(*) from w9inca where codgara = ? and codlott = ? and sezione in ('PA','RA')", new Object[] { codgara, codlott });
	...
	private void validazioneFinanziamenti(Long codgara, Long codlott, Long num, List<Object> listaControlli, Boolean adesioneAccordoQuadro) throws GestoreException {
		...
		try {
			...
			String flagEnteSpeciale = (String) this.sqlManager.getObject("select flag_ente_speciale from w9lott where codgara = ? and codlott = ?", new Object[] { codgara, codlott });
			...
			if (datiW9FINA != null && datiW9FINA.size() > 0) {
				for (int i = 0; i < datiW9FINA.size(); i++) {
					...
					} else if (importoFinanz <= 0) {
						listaControlliW9FINA
								.add(((new Object[] { "E", pagina, this.getDescrizioneCampo(nomeTabella, "IMPORTO_FINANZIAMENTO"), "L'importo finanziamento deve essere maggiore di zero" })));
					}
				}
			} else {
				if (adesioneAccordoQuadro) {
	...
	private void validazione2(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			...
			if (datiW9INIZ != null && datiW9INIZ.size() > 0) {
				...
				Date dataVerbAggiudicazione = this.getDataVerbaleAggiudicazione(codgara, codlott);
				...
				if (dataApprovProgEsec != null && dataInizProgEsec != null) {
					if (dataApprovProgEsec.getTime() <= dataInizProgEsec.getTime()) {
						String messaggio = this.getMessaggioConfrontoDate(dataApprovProgEsec, ">", dataStipula, "W9INIZ", "DATA_INI_PROG_ESEC", null);
		...
		try {
			Long conteggiow9Missic = (Long) this.sqlManager.getObject("select count(*) from W9MISSIC where CODGARA=? and CODLOTT=? and NUM_MISS=?", new Object[] { codgara, codlott, num });
		...
		// Incarichi professionali
		try {
			Long conteggiow9inca = (Long) this.sqlManager.getObject("select count(*) from w9inca where codgara = ? and codlott = ? and sezione = 'IN'", new Object[] { codgara, codlott });
			if (conteggiow9inca != null && conteggiow9inca.longValue() != 0) {
	...
	private void validazione3(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			...
			if (datiW9AVAN != null && datiW9AVAN.size() > 0) {
				Date dataVerbAggiudicazione = this.getDataVerbaleAggiudicazione(codgara, codlott);
				Double importoComplAppalto = (Double) this.sqlManager.getObject("select importo_compl_appalto from w9appa where codgara = ? and codlott = ? and num_appa = 1", new Object[] { codgara,
						codlott });
	...
	private void validazione101(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			...
			String selectW9IMPRESESingole = "select " + " partecip, " // 0
					+ " num_impr, " // 1
					+ " codimp " // 2
			...
			for (int k = 0; k < datiW9IMPRESESingole.size(); k++) {
				...
				// ottenuta la lista le assegno un titoloImprSingola da
				// visualizzare
	...
	private void validazione4(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			String selectW9CONC = "select " + " intantic, " // 0
					+ " id_motivo_interr, " // 1
					+ " id_motivo_risol," // 2
					+ " data_risoluzione, " // 3
					+ " flag_oneri, " // 4
			...
			if (datiW9CONC != null && datiW9CONC.size() > 0) {
				...
				if (intantic != null && "1".equals(intantic)) {
					...
					if (idMotivoInterr != null) {
						if (dataRisoluzione == null) {
							...
							Date dataStipula = this.getDataStipulaContratto(codgara, codlott);
							if (dataStipula != null) {
								if (dataRisoluzione.getTime() <= dataStipula.getTime()) {
									String messaggio = this.getMessaggioConfrontoDate(dataRisoluzione, ">", dataStipula, "W9INIZ", "DATA_STIPULA", new Long(CostantiW9.INIZIO_CONTRATTO_SOPRA_SOGLIA));
					...
					Double importoComplIntervento = (Double) this.sqlManager.getObject("select importo_compl_intervento from w9appa where codgara = ? and codlott = ? and num_appa = 1", new Object[] {
							codgara, codlott });
				...
				if (SqlManager.getValueFromVectorParam(datiW9CONC, 9).getValue() == null) {
				...
				if (dataUltimazione == null) {
					...
					if (dataVerbAggiudicazione != null && (dataUltimazione.getTime() <= dataVerbAggiudicazione.getTime())) {
				...
				if (numeroInfortumi == null) {
					...
					if (numeroInfortumi.longValue() > 9) {
	...
	private void validazione5(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			...
			String selectW9COLL = "select " + " data_regolare_esec, " // 0
			...
			if (datiW9COLL != null && datiW9COLL.size() > 0) {
				Date dataUltimazione = (Date) this.sqlManager.getObject("select data_ultimazione from w9conc where codgara = ? and codlott = ?", new Object[] { codgara, codlott });

				// DATA_VERB_INIZIO.W9INIZ
				Date dataEffettivoInizioLavori = (Date) this.sqlManager.getObject("select data_verb_inizio from w9iniz where codgara=? and codlott=? and num_iniz=? ", new Object[] { codgara, codlott,
						new Long(1) });

				Vector<?> datiW9APPA = this.sqlManager.getVector("select data_verb_aggiudicazione, IMPORTO_COMPL_INTERVENTO " + "from w9appa where codgara = ? and codlott = ?", new Object[] {
						codgara, codlott });

				Date dataVerbAggiudicazione = (Date) SqlManager.getValueFromVectorParam(datiW9APPA, 0).getValue();
				Double importoComplessivoIntervento = (Double) SqlManager.getValueFromVectorParam(datiW9APPA, 1).getValue();

				String tipoContratto = (String) this.sqlManager.getObject("select tipo_contratto from w9lott where codgara = ? and codlott =?", new Object[] { codgara, codlott });

				// Modalita' di collaudo
				Long modalitaCollaudo = (Long) SqlManager.getValueFromVectorParam(datiW9COLL, 2).getValue();

				// Data del certificato di regolare esecuzione e Data redazione
				// del certificato
				// di collaudo devono essere valorizzate in alternativa
				Date dataRegolareEsec = (Date) SqlManager.getValueFromVectorParam(datiW9COLL, 0).getValue();

				if ((modalitaCollaudo == null && dataRegolareEsec == null) || (modalitaCollaudo != null && dataRegolareEsec != null)) {
					String descrizione = this.getDescrizioneCampo(nomeTabella, "MODO_COLLAUDO") + ", " + this.getDescrizioneCampo(nomeTabella, "DATA_REGOLARE_ESEC");
					listaControlli.add(((new Object[] { "E", pagina, descrizione,
							"Specificare data del certificato di regolare esecuzione o le modalit&agrave; " + "del collaudo tecnico-amministrativo" })));
				}

				/*
				 * if (modalitaCollaudo != null && dataRegolareEsec != null) {
				 * String descrizione = this.getDescrizioneCampo(nomeTabella,
				 * "MODO_COLLAUDO") + ", " +
				 * this.getDescrizioneCampo(nomeTabella, "DATA_REGOLARE_ESEC");
				 * listaControlli.add(((new Object[] { "E", pagina, descrizione,
				 * "Collaudo tecnico-amministrativo non previsto in presenza di certificazione di regolare esecuzione"
				 * }))); }
				 */
				...
				// Modalita' collaudo tecnico amministrativo
				if (importoComplessivoIntervento != null && importoComplessivoIntervento.doubleValue() <= 500000 && ("L".equals(tipoContratto) || "CL".equals(tipoContratto))) {
					this.addAvviso(listaControlli, nomeTabella, "MODO_COLLAUDO", "W", pagina, "Redigere certificato di collaudo in luogo del certificato di regolare esecuzione");
				}

				// Data nomina collaudatore/commissione
				Date dataNominaColl = (Date) SqlManager.getValueFromVectorParam(datiW9COLL, 3).getValue();
				if (modalitaCollaudo != null && dataNominaColl == null) {
				...
				if (modalitaCollaudo != null && dataInizOper == null) {
				...
				if (modalitaCollaudo != null && dataCertCollaudo == null) {
				...
				if (importoComplessivoIntervento != null && importoComplessivoIntervento.doubleValue() <= 500000 && ("L".equals(tipoContratto) || "CL".equals(tipoContratto))) {
					this.addAvviso(listaControlli, nomeTabella, "DATA_CERT_COLLAUDO", "W", pagina, "Redigere il certificato di regolare esecuzione in luogo del certificato di collaudo");
				}
				...
				if (dataDelibera != null && dataCertCollaudo != null) {
					if (dataDelibera.getTime() < dataCertCollaudo.getTime()) {
						String messaggio = this.getMessaggioConfrontoDate(dataDelibera, ">=", dataVerbAggiudicazione, "W9COLL", "DATA_CERT_COLLAUDO", null);
		...
		try {
			...
			String selectW9COLL = "select " + " amm_num_definite, " // 0
					+ " amm_num_dadef, " // 1
					+ " amm_importo_rich, " // 2
					+ " amm_importo_def, " // 3
					+ " arb_num_definite, " // 4
		...
		// incaricati
		try {
			Long conteggiow9inca = (Long) this.sqlManager.getObject("select count(*) from w9inca where codgara = ? and codlott = ? and sezione = 'CO'", new Object[] { codgara, codlott });
			// if (conteggiow9inca == null
			// || (conteggiow9inca != null && (new
			// Long(0)).equals(conteggiow9inca)))
			// {
	...
	private void validazione6(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			...
			Date dataVerbInizio = (Date) this.sqlManager.getObject("select DATA_VERB_INIZIO from W9INIZ where CODGARA=? and CODLOTT=? and NUM_INIZ=1", new Object[] { codgara, codlott });
	...
	private void validazione7(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		if (!listaControlli.isEmpty()) {
			this.setTitolo(listaControlli, "Variante contratto");
		}
		...
		try {
			...
			if (datiW9VARI != null && datiW9VARI.size() > 0) {
				...
				if (dataVerbApprVariante == null) {
					...
					// Data effettivo inizio lavori
					Date dataVerbInizio = (Date) this.sqlManager.getObject("select DATA_VERB_INIZIO from w9iniz where codgara=? and codlott=?", new Object[] { codgara, codlott });
					...
					if (dataVerbInizio != null) {
						if (dataVerbApprVariante.getTime() <= dataVerbInizio.getTime()) {
							...
							this.addAvviso(listaControlli, nomeTabella, "DATA_VERB_APPR", "E", pagina, messaggio);
						}
					}
				}
				// Motivazioni
				...
				if (StringUtils.isEmpty(altreMotivazioni)) {
					// Se altre motivazioni e' vuoto controllo che ci sia almeno una riga in W9MOTI
				...
				Date dataEffettivoInizioLFS = (Date)
				  this.sqlManager.getObject("select DATA_VERB_INIZIO from W9INIZ where CODGARA=? and CODLOTT=?",
				      new Object[] { codgara, codlott });
				...
				pagina = "Quadro economico della variante";
        ...
        if ((conteggio != null && conteggio.longValue() == 1) && StringUtils.isEmpty(altreMotivazioni)) {
          if (StringUtils.isEmpty(cigNuovaProcedura)) {
            this.addAvviso(listaControlli, nomeTabella, "CIG_NUOVA_PROC", "E", pagina, 
            ...
            if (!UtilitySITAT.isCigValido(cigNuovaProcedura)) {
			...
			throw new GestoreException("Errore nella lettura delle informazioni relative alla variante", "validazione7", e);
		}
	...
	private void validazione9(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			String selectW9SUBA = "select " + " data_autorizzazione, " // 0
					+ " importo_presunto, " // 1
					+ " importo_effettivo, " // 2
					+ " id_categoria, " // 3
					+ " id_cpv, " // 4
			...
			if (datiW9SUBA != null && datiW9SUBA.size() > 0) {
				...
				if (isStringaValorizzata(codimpAggiudicatrice)) {
					...
					Long numImpreseAgg = (Long) this.sqlManager.getObject("select count(*) from w9aggi where codgara=? and codlott=? and num_appa=1 and codimp=?", new Object[] { codgara, codlott,
    ...
    // Quinta parte: validazione finanziamenti
		try {
		  ...
		  throw new GestoreException("Errore nell'estrazione del tipo di contratto della adesione all'accordo quadro (quinta parte)", "validazione12", se);
		}
	 ...
	 *             SQLException
	 */
	private Date getDataVerbaleAggiudicazione(final Long codgara, final Long codlott) throws SQLException {
		return (Date) this.sqlManager.getObject("select data_verb_aggiudicazione from w9appa where codgara = ? and codlott = ? and num_appa = 1", new Object[] { codgara, codlott });
	}
	 ...
	 *             SQLException
	 */
	private Date getDataStipulaContratto(final Long codgara, final Long codlott) throws SQLException {
		return (Date) this.sqlManager.getObject("select data_stipula from w9iniz where codgara = ? and codlott = ?", new Object[] { codgara, codlott });
	}

	/**
	 * Validazione dei dati per il flusso Gare enti nazionali o sotto i 40.000
	 * euro.
	 * 
	 * @param codgara
	 *            Codice della gara
	 ...
	 *             GestoreException
	 */
	private void validazione983(final Long codgara, final List<Object> listaControlli) throws GestoreException {
		if (logger.isDebugEnabled()) {
			logger.debug("validazione983: inizio metodo");
		}
		String nomeTabella = "W9GARA_ENTINAZ";
		String pagina = "";

		try {
			String selectW9GARAENTINAZ = "select OGGETTO, IDAVGARA, IMPORTO_GARA, " // 0,
																					// 1,
																					// 2
					+ "NLOTTI, TIPO_APP, DGURI, DSCADE " // 3, 4, 5, 6
					+ "from W9GARA_ENTINAZ where CODGARA = ? ";

			HashMap<?, ?> datiW9GaraEntinaz = this.sqlManager.getHashMap(selectW9GARAENTINAZ, new Object[] { codgara });
			if (datiW9GaraEntinaz != null && !datiW9GaraEntinaz.isEmpty()) {
				pagina = "Dati generali della gara";

				// Campo W9GARA_ENTINAZ.OGGETTO
				JdbcParametro jdbcPar = (JdbcParametro) datiW9GaraEntinaz.get("W9GARA_ENTINAZ.OGGETTO");
				if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "OGGETTO", pagina);
				}
				// Campo W9GARA_ENTINAZ.IDAVGARA
				jdbcPar = (JdbcParametro) datiW9GaraEntinaz.get("W9GARA_ENTINAZ.IDAVGARA");
				if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "IDAVGARA", pagina);
				}
				// Campo W9GARA_ENTINAZ.IMPORTO_GARA
				jdbcPar = (JdbcParametro) datiW9GaraEntinaz.get("W9GARA_ENTINAZ.IMPORTO_GARA");
				if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "IMPORTO_GARA", pagina);
				}
				// Campo W9GARA_ENTINAZ.NLOTTI
				jdbcPar = (JdbcParametro) datiW9GaraEntinaz.get("W9GARA_ENTINAZ.NLOTTI");
				if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "NLOTTI", pagina);
				} else {
					Long numeroLottiEsiti = (Long) this.sqlManager.getObject("select count(*) from W9LOTT_ENTINAZ where CODGARA = ?", new Object[] { codgara });
					if (((Long) jdbcPar.getValue()).longValue() != numeroLottiEsiti.longValue()) {
						String messaggio = this.getDescrizioneCampo(nomeTabella, "NLOTTI") + ": indicare il numero di lotti ed esiti effettivi ";
						listaControlli.add(((new Object[] { "E", "Dati generali della gara", "Dati generali della gara", messaggio })));
					}
				}
				// Campo W9GARA_ENTINAZ.TIPO_APP
				jdbcPar = (JdbcParametro) datiW9GaraEntinaz.get("W9GARA_ENTINAZ.TIPO_APP");
				if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "TIPO_APP", pagina);
				}

				// Count delle occorrenze in W9DOCGARA (numero dei documenti
				// allegati al bando)
				Long countW9DOCGARA = (Long) this.sqlManager.getObject("select count(*) from W9DOCGARA where CODGARA = ?", new Object[] { codgara });

				if (countW9DOCGARA != null && countW9DOCGARA.longValue() > 0) {
					// Campo W9GARA_ENTINAZ.DGURI
					jdbcPar = (JdbcParametro) datiW9GaraEntinaz.get("W9GARA_ENTINAZ.DGURI");
					if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
						this.addCampoObbligatorio(listaControlli, nomeTabella, "DGURI", pagina);
					}
					// Campo W9GARA_ENTINAZ.DSCADE
					jdbcPar = (JdbcParametro) datiW9GaraEntinaz.get("W9GARA_ENTINAZ.DSCADE");
					if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
				...
				if (countW9DOCGARA != null && countW9DOCGARA.longValue() > 0) {
					...
					if (conteggio != null && (countW9DOCGARA.longValue() != conteggio.longValue())) {
						String messaggio = "Indicare un file PDF per ogni titolo indicato";
						listaControlli.add(((new Object[] { "E", "Bando", "Bando", messaggio })));
					}
				}

				// Pagina Lotti ed esiti
				String selectW9lottEntinaz = "select " + " CODLOTT, OGGETTO, IMPORTO_TOT, CIG, " // 0,
																									// 1,
																									// 2,
																									// 3
						+ " IMP_AGG, DATA_VERB_AGGIUDICAZIONE, IMPORTO_AGGIUDICAZIONE, " // 3,
																							// 4,
																							// 5,
																							// 6
						+ " (select 1 from W9LOTT_ENTINAZ w2 " + "where w2.CODGARA = w1.CODGARA " + "and w2.CODLOTT = w1.CODLOTT " + "and w2.FILE_ALLEGATO is not null) as CTRLBLOB " // 7
						+ " from W9LOTT_ENTINAZ w1 where w1.CODGARA = ? order by w1.CODLOTT asc";

				List<?> listaHashMap = this.sqlManager.getListHashMap(selectW9lottEntinaz, new Object[] { codgara });
				if (listaHashMap != null && listaHashMap.size() > 0) {
					pagina = "Lotti ed esiti";
					nomeTabella = "W9LOTT_ENTINAZ";

					Long codiceLotto = null;

					List<Object> listaControlliLotti = new Vector<Object>();

					for (int ii = 0; ii < listaHashMap.size(); ii++) {
						HashMap<?, ?> datiW9LottEntinaz = (HashMap<?, ?>) listaHashMap.get(ii);
						codiceLotto = (Long) ((JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.CODLOTT")).getValue();
						// Campo OGGETTO del lotto
						jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.OGGETTO");
						if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
							this.addCampoObbligatorio(listaControlliLotti, nomeTabella, "OGGETTO", pagina);
						}
						// Campo IMPORTO_TOT del lotto
						jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.IMPORTO_TOT");
						if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
							this.addCampoObbligatorio(listaControlliLotti, nomeTabella, "IMPORTO_TOT", pagina);
						}
						// Campo CIG del lotto
						jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.CIG");
						if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
							this.addCampoObbligatorio(listaControlliLotti, nomeTabella, "CIG", pagina);
						}
						// Flag di controllo per esistenza del PDF allegato al
						// lotto ed esito
						boolean pdfAllegatoLottoEsito = true;
						jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("CTRLBLOB");
						if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
							pdfAllegatoLottoEsito = false;
						}

						if (pdfAllegatoLottoEsito) {
							// Campo IMP_AGG del lotto
							jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.IMP_AGG");
							if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
								this.addCampoObbligatorio(listaControlliLotti, nomeTabella, "IMP_AGG", pagina);
							}
							// Campo DATA_VERB_AGGIUDICAZIONE
							jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.DATA_VERB_AGGIUDICAZIONE");
							if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
								this.addCampoObbligatorio(listaControlliLotti, nomeTabella, "DATA_VERB_AGGIUDICAZIONE", pagina);
							}
							// Campo IMPORTO_AGGIUDICAZIONE
							jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.IMPORTO_AGGIUDICAZIONE");
							if (jdbcPar == null || (jdbcPar != null && jdbcPar.getValue() == null)) {
								this.addCampoObbligatorio(listaControlliLotti, nomeTabella, "IMPORTO_AGGIUDICAZIONE", pagina);
							}
						} else {
							// Campo IMP_AGG del lotto
							jdbcPar = (JdbcParametro) datiW9LottEntinaz.get("W9LOTT_ENTINAZ.IMP_AGG");
							if (jdbcPar != null && jdbcPar.getValue() != null) {
								String messaggio = "Indicare un file PDF al lotto ed esito numero " + codiceLotto;
								listaControlliLotti.add(((new Object[] { "E", "Lotto ed esito", "Lotto ed esito", messaggio })));
							}
						}
					}

					if (!listaControlliLotti.isEmpty()) {
						String titolo = "Dati del lotto n. " + codiceLotto.toString();
						this.setTitolo(listaControlli, titolo);
						for (int j = 0; j < listaControlliLotti.size(); j++) {
							listaControlli.add(listaControlliLotti.get(j));
						}
					}
				}
			}
		} catch (SQLException e) {
			throw new GestoreException("Errore nella lettura delle informazioni relative alla flusso per " + "Gare per enti nazionali o sotto i 40.000 euro", "validazione983", e);
		}
		if (logger.isDebugEnabled()) {
			logger.debug("validazione983: fine metodo");
		}
	}
		...
		try {
			...
			if (datiW9CONC != null && datiW9CONC.size() > 0) {
				...
				if (intantic != null && "1".equals(intantic)) {
					...
					Double importoComplIntervento = (Double) this.sqlManager.getObject("select importo_compl_intervento from w9appa where codgara = ? and codlott = ?",
							new Object[] { codgara, codlott });
					if (oneriRisoluzione != null && importoComplIntervento != null) {
						if (oneriRisoluzione.doubleValue() >= importoComplIntervento.doubleValue()) {
							String messaggio = this.getMessaggioControntoImporti(oneriRisoluzione, "<", importoComplIntervento, "W9APPA", "IMPORTO_COMPL_INTERVENTO", new Long(
				...
				if (dataUltimazione == null) {
					...
					Date dataVerbAgg = this.getDataVerbaleAggiudicazione(codgara, codlott);
					if (dataVerbAgg != null) {
	...
	private void validazioneW9SIC(Long codgara, Long codlott, Long num, List<Object> listaControlli, String pagina) throws SQLException {
		...
		List<?> datiW9SIC = this.sqlManager.getVector("select pianscic, dirope, tutor from w9sic where codgara=? and codlott=? and num_sic=?",
		    new Object[] { codgara, codlott, num });
	...
	private void validazione987(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			...
			if (datiW9APPA != null && datiW9APPA.size() > 0) {
				...
				// Incarichi professionali
				try {
					Long conteggiow9inca = (Long) this.sqlManager.getObject("select count(*) from w9inca where codgara = ? and codlott = ? and sezione in ('RS','RE')",
							new Object[] { codgara, codlott });
	...
	private void validazione998(Long codgara, Long codlott, Long num, List<Object> listaControlli) throws GestoreException {
		...
		try {
			String selectW9CANT = "select diniz, dlav, indcan, prov, comune" + " from w9cant where codgara = ? and codlott = ? and num_cant = ?";
	...
	private void validazione984(Long codgara, Long codlott, List<Object> listaControlli) throws GestoreException {
		...
		String pagina = "Comunicazione esito";

		try {
			Vector<?> datiW9ESITO = this.sqlManager.getVector("select esito_procedura, data_verb_aggiudicazione, importo_aggiudicazione " + "from w9esito where codgara=? and codlott=?", new Object[] {
					codgara, codlott });
			if (datiW9ESITO != null && datiW9ESITO.size() > 0) {
				...
				if (esitoProcedura == null) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "ESITO_PROCEDURA", pagina);
				} else {
	...
	private void validazione988(Long codgara, Long codlotto, List<Object> listaControlli, String profiloAttivo) throws GestoreException {
        ...
        // Durata accordo quadro
        if (tipoApp != null && ((new Long(9)).equals(tipoApp) || (new Long(17)).equals(tipoApp) || (new Long(18)).equals(tipoApp))) {
          if (SqlManager.getValueFromVectorParam(datiW9GARA, 22).getValue() == null) {
            ...
            this.addAvviso(listaControlli, nomeTabella, "DURATA_ACCQUADRO", "E", pagina, "Il valore del campo deve essere maggiore di zero");
				...
				if (SqlManager.getValueFromVectorParam(datiW9GARA, 18).getValue() == null) {
	...
	private void validazioneImpreseAggiudicatarie(Long codgara, Long codlott, Long numappa, List<Object> listaControlli) throws GestoreException {
      ...
      String selectNumRaggW9AGGI = "select distinct id_gruppo,id_tipoagg from w9aggi where codgara = ? and codlott = ? and num_appa = ? order by id_gruppo, id_tipoagg";
			...
			for (int j = 0; j < numeriRaggruppamentiW9AGGI.size(); j++) {
				...
				boolean isImpresaSingola = (new Long(3)).equals(tipoAggiudicataria);
				...
				String selectW9AGGI = "select w1.num_aggi, " // 0
						...
						+ " where codgara = ? and codlott = ? and num_appa = ? " + andIdGruppo;
					...
					if (datiW9AGGI != null && datiW9AGGI.size() > 0) {
						...
						for (int i = 0; i < datiW9AGGI.size(); i++) {
							...
							if (idTipoAgg == null) {
								...
								if (new Long(3).equals(idTipoAgg)) {
									// Impresa singola
							...
							// controllo tipologia per imprese singole (dev'essere 3)
							if (isImpresaSingola) {
								if (!(new Long(3).equals(idTipoAgg))) {
	...
	private void validazioneMisureAggiuntiveSicurezza(final Long codgara, final Long codlott, final Long num, final List<Object> listaControlli) throws GestoreException {
		...
		try {
			...
			String sqlW9MISSIC = "select DESCMIS from W9MISSIC where CODGARA=? and CODLOTT=? and NUM_MISS=?";
	 ...
	 *             GestoreException
	 */
	private void validazioneImpresa(List<Object> listaControlli, String pagina, String codimp) throws GestoreException {
		if (logger.isDebugEnabled()) {
			...
			if (datiINTTRI != null && datiINTTRI.size() > 0) {
				...
				if (isANNINTuguale1 && isSEZINTdiverso0607 && isTCPINTdiverso01) {
					...
					if ((!isStringaValorizzata(comint)) && (!isStringaValorizzata(nuts))) {
						...
						listaControlli.add(((new Object[] { "E", pagina, descrizione, "Valorizzare uno dei due campi" })));
					}

					if (isStringaValorizzata(comint) && isStringaValorizzata(nuts)) {
						String descrizione = this.getDescrizioneCampo(nomeTabella, "COMINT") + ", " + this.getDescrizioneCampo(nomeTabella, "NUTS");
						listaControlli.add(((new Object[] { "E", pagina, descrizione, "Valorizzare solo uno dei due campi" })));
					}
	...
	private void validazioneLotto(final Long codgara, final Long codlott, final List<Object> listaControlli, String profiloAttivo) throws GestoreException {
		...
		try {
			...
			if (datiW9LOTT != null && datiW9LOTT.size() > 0) {
				...
				int indiceDatiW9Lott = 0;

				// Oggetto del lotto
				String str = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				if (!isStringaValorizzata(str)) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "OGGETTO", pagina);
				}

				indiceDatiW9Lott++; // indiceDatiW9Lott = 1
				// Numero del lotto
				Long nlotto = (Long) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				if (nlotto == null) {
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 2
				// Lotto derivante da contratto precedente ?
				String comcon = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				...
				if (comcon != null && "1".equals(comcon)) {
					indiceDatiW9Lott++; // indiceDatiW9Lott = 3

					// Motivo del completamento
					if (SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue() == null) {
						this.addCampoObbligatorio(listaControlli, nomeTabella, "DESCOM", pagina);
					}

					indiceDatiW9Lott++; // indiceDatiW9Lott = 4
					// CIG del contratto da cui deriva il lotto
					str = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
					if (!isStringaValorizzata(str)) {
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 5
				// Somma urgenza ?
				//str = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				//if (!isStringaValorizzata(str)) {
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 6
				// Tipo contratto
				String tipoContratto = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				if (!isStringaValorizzata(tipoContratto)) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "TIPO_CONTRATTO", pagina);
				}

				indiceDatiW9Lott = indiceDatiW9Lott + 2; // indiceDatiW9Lott = 8
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 9
				...
				indiceDatiW9Lott = indiceDatiW9Lott + 2; // indiceDatiW9Lott = 11
				// Procedura di scelta del contraente
				Long idSceltaContraente = (Long) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				if (idSceltaContraente == null) {
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 12
				// Criterio di aggiudicazione
				// Valorizzare se W3ID_CONDI != 2, 5, 7, 9, 10, 11, 14, 16,17, 18, 23, 24, 28
				Long numeroCondizioni = (Long) this.sqlManager.getObject("select count(*) from W9COND where CODGARA=? and CODLOTT=? "
				...
				if ((!isAAQ && (versioneSimog == 2 || (versioneSimog == 1 && numeroCondizioni == 0))) && 
				    SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue() == null) {
        ...
        if (SqlManager.getValueFromVectorParam(datiW9LOTT, 27).getValue() != null) {
          ...
          if (cuiint.length() == 21 || cuiint.length() == 20) {
            ...
            listaControlli.add(((new Object[] { "E", "Codice CUI", "Codice CUI", "Il codice CUI non &egrave; valido" })));
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 13
				// L'importo e' disponibile ?
				str = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				
				indiceDatiW9Lott++; // indiceDatiW9Lott = 14
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 15
				// Importo per l'attuazione della sicurezza
				if (SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue() == null) {
					this.addCampoObbligatorio(listaControlli, nomeTabella, "IMPORTO_ATTUAZIONE_SICUREZZA", pagina);
				}
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 16
				// Codice CUP
				if (tipoContratto != null && ("L".equals(tipoContratto) || "CL".equals(tipoContratto))) {
				  if (!UtilitySITAT.isSAQ(codgara, sqlManager)) {
  				  ...
  				  if (isManutezioneNonOrdinaria.longValue() > 0) {
  				    ...
  				    if (! isStringaValorizzata(str)) {
  				      //this.addCampoObbligatorio(listaControlli, nomeTabella, "Codice CUP / Esente CUP", pagina);
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott =17
				// Codice CPV
				str = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				if (!isStringaValorizzata(str)) {
          ...
          listaControlli.add(((new Object[] { "E", pagina, this.getDescrizioneCampo(nomeTabella, "CPV"), "Codice CPV non valido" })));
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 18
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 19
				Double importoTot = (Double) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();

				indiceDatiW9Lott++; // indiceDatiW9Lott = 20
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 21
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 22
				// Luogo ISTAT e NUTS
				String luogoISTAT = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				indiceDatiW9Lott++; // indiceDatiW9Lott = 23
				String luogoNUTS = (String) SqlManager.getValueFromVectorParam(datiW9LOTT, indiceDatiW9Lott).getValue();
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 24
				...
				indiceDatiW9Lott++; // indiceDatiW9Lott = 25
