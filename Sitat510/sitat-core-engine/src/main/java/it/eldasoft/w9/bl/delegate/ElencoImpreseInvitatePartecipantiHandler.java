...
public class ElencoImpreseInvitatePartecipantiHandler extends AbstractRequestHandler {
  ...
  protected void managePrimoInvio(XmlObject documento,
    ...
    if (existsFase(fase.getW3FACIG(), CostantiW9.ELENCO_IMPRESE_INVITATE_PARTECIPANTI)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto con fase elenco imprese invitate/partecipanti gia' presente nel DB");
      this.setEsito(
          datiAggiornamento,
          StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di un Elenco imprese invitate/partecipanti per questo lotto gia' presente in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  protected void manageRettifica(XmlObject documento,
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di rettifica elenco imprese invitate/partecipanti ha un cig ("
          + fase.getW3FACIG() + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
