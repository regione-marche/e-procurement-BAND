...
public class XMLRLOManager {
	 ...
	 *             IOException
	 */
	public TrasferimentoDatiType getTrasferimentoDati(final Long codgara, final Long codlott, final Long fase_esecuzione, final Long num) throws GestoreException {
		...
		try {
			...
			setContratti(contratti, codgara, codlott, fase_esecuzione, num);
	...
	private void setTipiAppaltoForn(final FASEAGGIUDICAZIONEType aggiudicazione, final Long codgara, final Long codlott) throws GestoreException {
		...
		try {
			List<?> datiW9APPAFORN = sqlManager.getListVector(selectW9APPAFORN, new Object[] { codgara, codlott });
			...
			if (datiW9APPALAV != null && datiW9APPALAV.size() > 0) {
				...
				while (iterator.hasNext()) {
					...
					if (verificaEsistenzaValore(riga.get(0))) {
						...
						if (valoreDaConvertire.equals("6")) {//Costruzione
							lavoro.setCODICELAVORO(TipologiaLavoriType.X_1);
						} else if (valoreDaConvertire.equals("7")) {//Demolizione
							lavoro.setCODICELAVORO(TipologiaLavoriType.X_2);
						} else if (valoreDaConvertire.equals("8")) {//Recupero
							lavoro.setCODICELAVORO(TipologiaLavoriType.X_3);
						} else if (valoreDaConvertire.equals("9")) {//Ristrutturazione
							lavoro.setCODICELAVORO(TipologiaLavoriType.X_4);
						} else if (valoreDaConvertire.equals("10") || valoreDaConvertire.equals("9")) {//Restauro
							lavoro.setCODICELAVORO(TipologiaLavoriType.X_5);
						} else if (valoreDaConvertire.equals("12") || valoreDaConvertire.equals("13")) {//Manutenzione
							lavoro.setCODICELAVORO(TipologiaLavoriType.X_6);
						} else {
							lavoro.setCODICELAVORO(TipologiaLavoriType.Enum.forString(riga.get(0).toString()));
						}
					}
				}
			}
		...
		try {
			...
			if (datiW9COND != null && datiW9COND.size() > 0) {
				...
				while (iterator.hasNext()) {
					...
					if (verificaEsistenzaValore(riga.get(0))) {
						condizione.setCODICECONDIZIONE(CondizioneGiustificazioneType.Enum.forString(riga.get(0).toString()));
					}
				}
			}
		} catch (SQLException e) {
		...
		try {
			...
			if (datiLOTTO != null && datiLOTTO.size() > 0) {
				...
				if (verificaEsistenzaValore(datiLOTTO.get(11))) {
					aggiudicazione.setCODICEPROCEDURAGARA(ProceduraGaraType.Enum.forString(StringUtils.leftPad(datiLOTTO.get(11).toString(), 2,"0")));
				}
				
				if (verificaEsistenzaValore(datiLOTTO.get(12))) {
					aggiudicazione.setCODICECRITERIOAGGIUDICAZIONE(CriterioAggiudicazioneType.Enum.forString(datiLOTTO.get(12).toString()));
				} 
		...
		try {
			...
			if (datiVARI != null && datiVARI.size() > 0) {
				...
				if (verificaEsistenzaValore(datiVARI.get(1))) {
					variante.setMOTIVAZIONE(MotivoVarianteType.Enum.forString(datiVARI.get(1).toString()));
				}
	 ...
	 *             IOException
	 */
	private void setContratti(APCONTRATTOMASTERType contratti, Long codgara, Long codlott, Long fase_esecuzione, Long num) throws GestoreException {
		...
		contratti.setUsername("franzonimirco");
	...
	private void validazioneAggiudicazione(TrasferimentoDatiType trasferimentoDati, List<Object> listaControlli) {
		...
		try {
			...
			if (trasferimentoDati.getContrattiArray(0).getCODICESETTORE().equals(TipologiaSettore.X_02)) {
			...
			if (trasferimentoDati.getContrattiArray(0).getCODICESETTORE().equals(TipologiaSettore.X_01) 
					&& aggiudicazione.getCODICECRITERIOAGGIUDICAZIONE().equals(CriterioAggiudicazioneType.X_2)
			...
			if(aggiudicazione.getCODICECRITERIOAGGIUDICAZIONE().equals(CriterioAggiudicazioneType.X_2)) {
