...
public class GestoreW9MISSIC extends AbstractGestorePreload {
  ...
  private static final String SQL_ESTRAZIONE_NOMIMP = "select IMPR.NOMEST from W9MISSIC, IMPR where CODGARA = ? AND CODLOTT = ? AND NUM_MISS = ? and IMPR.CODIMP=W9MISSIC.CODIMP";
  ...
  public void doBeforeBodyProcessing(PageContext page, String modoAperturaScheda)
    ...
    if (!UtilityTags.SCHEDA_MODO_INSERIMENTO.equals(modoAperturaScheda)) {
      ...
      try {
        ...
        //nummiss = (key.getColumnsBySuffix("NUM_MISS", true))[0].getValue().longValue();

        String nomeimpr = "";
        if (sqlManager.getObject(SQL_ESTRAZIONE_NOMIMP, new Object[] { codgara, codlott, nummiss }) != null) {
          nomeimpr = sqlManager.getObject(SQL_ESTRAZIONE_NOMIMP, new Object[] { codgara, codlott, nummiss }).toString();
        }
        ...
        Long ctrlBlob = (Long) sqlManager.getObject(sqlW9DOCFASE, new Object[] {
            codgara, codlott, new Long(CostantiW9.MISURE_AGGIUNTIVE_SICUREZZA), nummiss });
