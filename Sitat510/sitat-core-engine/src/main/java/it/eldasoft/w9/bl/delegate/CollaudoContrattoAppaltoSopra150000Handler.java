...
AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di primo invio ha un cig di un lotto non presente in DB (CIG = "
          + fase.getW3FACIG() + ")");
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di primo invio ha un cig  di un lotto non appaltato sul DB (CIG = "
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di fase collaudo di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }

    if (!this.existsAggiudicatari(fase)) {
      logger.error("La richiesta di primo invio ha un cig  di un lotto senza imprese aggiudicatarie sul DB(CIG = "
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di fase collaudo di un lotto senza imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }

    if (!this.existsInizio(fase)) {
      logger.error("La richiesta di primo invio ha un cig  di un lotto senza fase iniziale del contratto sul DB (CIG = "
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di fase collaudo di un lotto senza fase iniziale del contratto in banca dati");
      // si termina con questo errore
      return;
    }

    if (!this.existsConclusione(fase)) {
      logger.error("La richiesta di primo invio ha un cig  di un lotto senza fase conclusione del contratto sul DB("
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di fase collaudo di un lotto senza fase conclusione del contratto in banca dati");
      // si termina con questo errore
      return;
    }

    if (this.existsCollaudo(fase)) {
      logger.error("La richiesta di primo invio ha un cig di un lotto con collaudo gia' presente sul DB (CIG = "
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
  ...
  /**
   * Verifica se esiste in banca dati la fase di collaudo per il lotto indicato
  ...
  private boolean existsCollaudo(FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        "select count(w9coll.codgara) from w9coll, w9lott "
        + "where w9coll.codgara = w9lott.codgara and w9coll.codlott = w9lott.codlott "
        + "and w9coll.num_coll = 1 and w9lott.cig = ?",
        new Object[] { fase.getW3FACIG() });
    return numeroOccorrenze.intValue() > 0;
  }

  /**
   * Verifica se esiste in banca dati la fase conclusione per il lotto indicato
   * a partire dal cig fornito nel tracciato.
   * 
   * @param fase
   *        elemento fase del flusso
  ...
  private boolean existsConclusione(FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        ...
        + "and w9conc.num_conc = 1 and w9lott.cig = ?",
        new Object[] { fase.getW3FACIG() });
    return numeroOccorrenze.intValue() > 0;
  }
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di fase collaudo di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicatari(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di fase collaudo di un lotto senza imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsInizio(fase)) {
      logger.error("La richiesta di Rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto senza fase iniziale del contratto sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di fase collaudo di un lotto senza fase iniziale del contratto in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsConclusione(fase)) {
      logger.error("La richiesta di Rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto senza fase conclusione del contratto sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di fase collaudo di un lotto senza fase conclusione del contratto in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsCollaudo(fase)) {
      logger.error("La richiesta di Rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto con collaudo non presente sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (!ignoreWarnings) {
      ...
      if (!isS2 || isE1 || !isOrd) {
        logger.error("La richiesta di rettifica di collaudo contratto non previsto per il lotto con CIG=" + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Rettifica di collaudo contratto non previsto");
        // si termina con questo errore
        
        if (!isS2) {
          logger.error("La richiesta di rettifica ha il lotto inferiore a 40000");
        }
        if (isE1) {
          logger.error("La richiesta di rettifica ha il lotto escluso");
        }
        if (!isOrd) {
          logger.error("La richiesta di rettifica ha il lotto senza settori ordinari");
        }
        return;
      }
    }
  ...
  private void deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9coll where codgara = ? and codlott = ?", new Object[] {
            codGara, codLott }, 1);
    this.sqlManager.update(
        "delete from w9inca where codgara = ? and codlott = ? and sezione = 'CO'",
