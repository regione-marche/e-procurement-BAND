...
public class GestoreW9FLUSSI extends AbstractGestoreChiaveIDAutoincrementante {
	...
	public void postInsert(DataColumnContainer impl) throws GestoreException {
		...
		String profiloAttivo = (String)
		    this.getRequest().getSession().getAttribute(CostantiGenerali.PROFILO_ATTIVO);
		
		boolean isLoaderAppaltoEnable = geneManager.getProfili().checkProtec(profiloAttivo, "FUNZ", "VIS", "ALT.W9.INVIISIMOG");
					...
					if (numeroErrori == 0) {
						...
						if (xmlValidationError != null && xmlValidationError.size() > 0) {
							...
							if (isLoaderAppaltoEnable) {
								...
								if (tipoInvio == -1) {
									...
									if (CostantiSimog.TipiSchede.containsKey(valoreFlusso)) {
										exportXmlSimogManagerm.loaderAppalto(codiceGara, codiceLotto, valoreFlusso, progressivoFase, impl.getLong("W9FLUSSI.IDFLUSSO"), simoguser, simogpass, true);
									...
									//invio la fase
									erroriProxy = exportXmlSimogManagerm.loaderAppalto(codiceGara, codiceLotto, valoreFlusso, progressivoFase, impl.getLong("W9FLUSSI.IDFLUSSO"), simoguser, simogpass, false);	
								}
							} else {
							...
							if ((erroriProxy == null || (erroriProxy != null && erroriProxy.length == 0))
									&& (erroriDalServizioPubblicazione == null || (erroriDalServizioPubblicazione != null && erroriDalServizioPubblicazione.size() == 0))
							) {
								if (!isLoaderAppaltoEnable) {
									...
									// perche' la rettifica prevede la cancellazione delle occerrenze esistenti e quindi e' necessario
									// rivalorizzare i campi SITUAZIONE di W9GARA e W9LOTT.
									// Long tipoInvio = impl.getLong("TINVIO2");

									if (valoreFlusso.intValue() != CostantiW9.PROGRAMMA_TRIENNALE_FORNITURE_SERVIZI && valoreFlusso.intValue() != CostantiW9.PROGRAMMA_TRIENNALE_LAVORI
											&& valoreFlusso.intValue() != CostantiW9.PROGRAMMA_BIENNALE_ACQUISTI && valoreFlusso.intValue() != CostantiW9.PROGRAMMA_TRIENNALE_OPERE_PUBBLICHE
									    && valoreFlusso.intValue() != CostantiW9.PUBBLICAZIONE_AVVISO && valoreFlusso.intValue() != CostantiW9.GARE_ENTI_NAZIONALI_O_SOTTO_40000) {
								...
								// Aggiungo nel request il dettaglio degli errori del proxy per
								// visualizzarli nella pagina di destinazione
