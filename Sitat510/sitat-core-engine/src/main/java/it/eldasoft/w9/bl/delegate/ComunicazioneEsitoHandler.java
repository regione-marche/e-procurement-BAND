...
public class ComunicazioneEsitoHandler extends AbstractRequestHandler {
  ...
  protected void managePrimoInvio(final XmlObject documento,
    ...
    if (!this.existsLotto(fase)) {
      ...
      this.setEsito(
          ...
          "Primo invio di una Comunicazione esito con lotto non esistente in banca dati");
      return;
    }
    ...
    if (existsFase(fase.getW3FACIG(), CostantiW9.COMUNICAZIONE_ESITO)) {
      logger.error("La richiesta di primo invio ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto con fase comunicazione esito gia' presente nel DB");
      this.setEsito(
          datiAggiornamento,
          StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di una Comunicazione esito per questo lotto, gia' presente in banca dati");
  ...
  protected void manageRettifica(final XmlObject documento,
    ...
    if (!this.existsLotto(fase)) {
      ...
      this.setEsito(
          datiAggiornamento,
          StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di una Comunicazione esito con lotto non esistente in banca dati");
      return;
    }
    ...
    if (!existsFase(fase.getW3FACIG(), CostantiW9.COMUNICAZIONE_ESITO)) {
      logger.error("La richiesta di rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto senza fase comunicazione esito in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di una Comunicazione esito di un lotto senza una fase comunicazione esito in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  private void deleteDatiFlusso(final FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update("delete from w9pues where codgara = ? and codlott = ?",
        new Object[] { codGara, codLott });
