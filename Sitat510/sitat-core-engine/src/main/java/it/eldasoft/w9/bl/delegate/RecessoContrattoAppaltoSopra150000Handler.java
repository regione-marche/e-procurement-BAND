...
AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAggiudicazione(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di una Istanza di recesso di un appalto con aggiudicazione non esistente in banca dati");
      return;
    }
    ...
    if (!this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di un Recesso del contratto con appalto non esistente in banca dati");
      return;
    }

    if (this.existsRecesso(fase)) {
      logger.error("La richiesta di primo invio ha un NUM_RITA  ("
          + fase.getW3NUM5()
          + ") di un lotto gia' concluso sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di una Istanza di recesso di un appalto gia' presente in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      String tipo = (String) this.sqlManager.getObject(
          "select tipo_contratto from w9lott where cig = ?",
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsRecesso(fase)) {
      logger.error("La richiesta di rettifica ha un NUM_RITA  ("
          + fase.getW3NUM5()
          + ") non presente sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (!this.existsAggiudicazione(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di una aggiudicazione non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Rettifica di una Istanza di recesso di un appalto con aggiudicazione non esistente in banca dati");
      return;
    }
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          ...
          + ") di un lotto non appaltato sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Rettifica di un Recesso del contratto con appalto non esistente in banca dati");
      return;
    }
    // esiste lotto
    if (!this.existsLotto(fase)) {
    ...
    if (!ignoreWarnings) {
      ...
      if ((!isS2 && !isAII) || !isLavori || isE1 || isSAQ || !isOrd) {
        logger.error("La richiesta di rettifica istanza di recesso non previsto per il lotto con CIG="
            + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Rettifica istanza di recesso non previsto");
        
        if (!isS2 && !isAII) {
          logger.error("La richiesta di rettifica ha un lotto con importo lotto inferiore a 40000 e non e' un intervento per ricostruzione alluvione Lunigiana-Elba");
        }
        if (!isLavori) {
          logger.error("La richiesta di rettifica ha un lotto che non e' per lavori");
        }
        if (isE1) {
          logger.error("La richiesta di rettifica ha un lotto escluso");
        }
        if (isSAQ) {
          logger.error("La richiesta di rettifica ha un lotto che e' una stipula accordo quadro (TIPO_APP=9)");
        }
        if (!isOrd) {
          logger.error("La richiesta di rettifica ha un lotto senza settori ordinari");
        }
        return;
      }
    }
  ...
  private boolean existsRecesso(final FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        "select count(w9rita.codgara) from w9rita, w9lott "
        + "where w9rita.codgara = w9lott.codgara and w9rita.codlott = w9lott.codlott "
        + "and w9rita.num_rita = ? and w9lott.cig = ?", new Object[] {
            fase.getW3NUM5(), fase.getW3FACIG() });
