...
AbstractRequestHandler {
  ...
  protected void managePrimoInvio(XmlObject documento,
    ...
    if (!this.existsLotto(fase)) {
      ...
      "Primo invio di aggiudicazione semplificata appalto di un lotto non presente in banca dati");
    ...
    if (this.existsAppalto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Primo invio di aggiudicazione semplificata appalto di un lotto gia' appaltato in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica ha un cig di un lotto non appaltato sul DB (CIG = "
          + fase.getW3FACIG() + ")");
      this.setEsito(
          datiAggiornamento,
          StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di aggiudicazione semplificata appalto di un lotto non appaltato in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if (!isE1 && isS2) {
        ...
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
            "Rettifica di aggiudicazione semplificata non prevista");
        // si termina con questo errore
        return;
      }
    }
  ...
  private void manageIntegrazioneORettifica(
      ...
      SQLException {
    this.deleteDatiFlusso(doc.getRichiestaRichiestaRispostaSincronaAggiudicazioneAppaltoSotto150000().getFase());
  ...
  private void deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update("delete from w9appa where codgara = ? and codlott = ?",
        new Object[] { codGara, codLott }, 1);
    this.sqlManager.update("delete from w9aggi where codgara = ? and codlott = ?",
        new Object[] { codGara, codLott });

    this.sqlManager.update(
        "delete from w9inca where codgara = ? and codlott = ? and (sezione = ? or sezione = ?)",
        new Object[] { codGara, codLott, "RE", "RS" });
  }
