  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG() + ") privo di appalto");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di accordo bonario di un lotto privo di appalto in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicatari(fase)) {
      logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG() + ") privo di imprese aggiudicatarie sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di accordo bonario di un lotto privo di imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!(this.existsFase(fase.getW3FACIG(), CostantiW9.INIZIO_CONTRATTO_SOPRA_SOGLIA)
      ...
      logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG() + ") privo della fase iniziale del contratto o stipula accordo quadro sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di accordo bonario di un lotto privo della fase iniziale del contratto" +
    ...
    if (this.existsFase(fase.getW3FACIG(), CostantiW9.ACCORDO_BONARIO, fase.getW3NUM5())) {
      logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG() + ") con accordo bonario gia' presente sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
    ...
    if (!ignoreWarnings) {
      ...
      if (!isS2 || isE1 || isSAQ || !isOrd) {
        ...
        if (!isS2) {
          logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
              + fase.getW3FACIG() + ") con importo inferiore a 40000");
        }
        if (isSAQ) {
          logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
              + fase.getW3FACIG() + ") con stipula accordo quadro (TIPO_APP=9)");
        }
        if (isSAQ) {
          logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
              + fase.getW3FACIG() + ") escluso");
        }
        if (!isOrd) {
          logger.error("La richiesta di primo invio accordo bonario fa riferimento ad un lotto (cig="
              + fase.getW3FACIG() + ") senza settori oridnari");
        }
        // si termina con questo errore
        return;
      }
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          + fase.getW3FACIG() + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Rettifica accordo bonario di un lotto non presente in banca dati");
      // si termina con questo errore
      return;
    }

    if (!this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG() + ") privo di appalto");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Rettifica accordo bonario di un lotto privo di appalto in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsAggiudicatari(fase)) {
      logger.error("La richiesta di rettifica accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG() + ") privo di imprese aggiudicatarie sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Rettifica di accordo bonario di un lotto privo di imprese aggiudicatarie in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if ((this.existsFase(fase.getW3FACIG(), CostantiW9.INIZIO_CONTRATTO_SOPRA_SOGLIA)
        ...
        || this.existsFase(fase.getW3FACIG(), CostantiW9.STIPULA_ACCORDO_QUADRO))) {
      logger.error("La richiesta di rettifica accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG() + ") privo della fase inziale del contratto o stipula accordo quadro");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Rettifica di accordo bonario di un lotto privo della fase inziale del contratto" +
        " o stipula accordo quadro in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!this.existsFase(fase.getW3FACIG(), CostantiW9.ACCORDO_BONARIO, fase.getW3NUM5())) {
      logger.error("La richiesta di rettifica accordo bonario fa riferimento ad un lotto (cig="
          + fase.getW3FACIG()
          + ") privo del primo invio dell'accordo bonario stesso");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Rettifica di accordo bonario di un lotto avente gia' una fase conclusione in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if (!isS2 || isE1 || !isOrd) {
        logger.error("La richiesta di rettifica accordo bonario non previsto per il lotto con CIG="
            + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Rettifica di accordo bonario non previsto");
        if (!isS2) {
          logger.error("La richiesta di rettifica ha un lotto inferiore a 40000");
        }
        if (isE1) {
          logger.error("La richiesta di rettifica ha un lotto escluso");
        }
        if (isSAQ) {
          logger.error("La richiesta di rettifica fa riferimento ad un lotto (cig="
              + fase.getW3FACIG() + ") con stipula accordo quadro (TIPO_APP=9)");
        }
        if (!isOrd) {
          logger.error("La richiesta di rettifica ha un lotto senza settori oridnari");
        }
        // si termina con questo errore
        return;
      }
