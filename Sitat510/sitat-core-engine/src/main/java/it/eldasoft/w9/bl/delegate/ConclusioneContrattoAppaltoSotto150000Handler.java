...
AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsConclusione(fase)) {
      logger.error("La richiesta di rettifica ha un cig ("
          + fase.getW3FACIG()
          + ") di un lotto senza una fase conclusione sul DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      "Rettifica di conclusione appalto semplificata di un lotto senza una fase conclusione in banca dati");
      // si termina con questo errore
      return;
    }
    ...
    if (!ignoreWarnings) {
      ...
      if ((!isE1 && isS2 && isOrd) || isSAQ) {
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
        "Rettifica di conclusione appalto semplificata non prevista");
        
        if (!isE1 && isS2) {
          logger.error("La richiesta di rettifica ha un lotto con importo maggiore o uguale a 40000 e non escluso");
        }
        if (isSAQ) {
          logger.error("La richiesta di rettifica ha il lotto relativo ad una stipula accordo quadro (TIPO_APP=9)");
        }
        return;
      }
    }
