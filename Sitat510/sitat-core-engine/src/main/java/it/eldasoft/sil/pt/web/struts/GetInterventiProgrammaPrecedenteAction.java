    ...
    protected ActionForward runAction(ActionMapping mapping, ActionForm form,
      ...
      Long tipoProgramma = (Long) this.sqlManager.getObject("select TIPROG from PIATRI where CONTRI=?",
        ...
        if (idProgramma != null) {
          ...
          filtroIdProgramma = " and p.ID = '" + ((Vector) listaIdProgrammi.get(0)).get(0).toString() + "' ";
          request.setAttribute("idProg", ((Vector) listaIdProgrammi.get(0)).get(0).toString());
        }
              
                // Per gli interventi non riproposti 
                listaInterventi = this.sqlManager.getListVector(
                    "select i.CONTRI, i.CONINT, i.CUIINT, i.DESINT, i.TOTINT, coalesce(i.ANNRIF,0)+p.ANNTRI-1 as ANNO from INTTRI i, PIATRI p "
        ...
        if (annoIntervento != null) {
          if (("-1".equals(annoIntervento))) {
            ...
            filtroAnno = " and i.ANNRIF = " + annoIntervento;
          }
        }
        ...
        if ("RDP_RevisioneInterventiAnnoPrecedente".equalsIgnoreCase(tipoInterventi)) {
          ...
          listaInterventi = this.sqlManager.getListVector(
              "select i.CONTRI, i.CONINT, i.CUIINT, i.DESINT, i.TOTINT, coalesce(i.ANNRIF,0) + p.ANNTRI-1 as ANNO, f.conint AS F_CONINT "
          ...
          listaAnnualitaInterventiProgPrecedenti = sqlManager.getListVector(
              "select distinct(coalesce(i.ANNRIF,0)+p.ANNTRI-1) as ANNO,coalesce(i.ANNRIF,0) as ANNRIF " 
             + " from INTTRI i, PIATRI p, FABBISOGNI f, PTAPPROVAZIONI pt "
             + " where i.CONTRI in (select CONTRI from PIATRI where ANNTRI=? and CENINT=?) "
             + "   and i.CONTRI = p.CONTRI "
