...
public class CopiaPubblicazioneAction extends ListaAction {
    ...
    public ActionForward copiaPubblicazione(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            //if (descrizione.indexOf(" - Aggiornamento n.") != -1) {
            //  descrizione = descrizione.substring(0, descrizione.indexOf("Aggiornamento n."));
            //}
            //descrizione = descrizione + " - Aggiornamento n. ";
            ...
            // oppure dbms= geneManager.getSql().getTipoDB();
            // per non leggere da properties
            if ("ORA".equals(dbms)) {
                selectColumns = " select column_name,data_type from USER_TAB_COLUMNS where table_name= ? and data_type <> 'BLOB'";
            }
            if ("MSQ".equals(dbms)) {
                selectColumns = " SELECT CAST(name AS VARCHAR) FROM sys.columns WHERE object_id = OBJECT_ID(?) and name <> 'FILE_ALLEGATO'";
            }
            if ("DB2".equals(dbms)) {
            ...
            campiDaInserire.removeColumns(new String[] { "PIATRI.CONTRI", "PIATRI.ID", "PIATRI.NAPTRI", "PIATRI.DPAPPROV", "PIATRI.DAPTRI", "PIATRI.TITAPPROV", "PIATRI.URLAPPROV", "PIATRI.ID_RICEVUTO", "PIATRI.ID_GENERATO" });
            ...
            for (int i = 0; i < tabelleProgramma.length; i++) {
                // ripeto epr ogni tabella l'iter di Piatri
                // modificando sulo il contri
                ...
                if(tabelleProgramma[i].equals("INTTRI")){
                  ...
                  if (listaApprovazioniSorgente != null
                      && listaApprovazioniSorgente.size() > 0) {
                    ...
                    for (int rowAppr = 0; rowAppr < listaApprovazioniSorgente.size(); rowAppr++) {
                      ...
                      newNumappr = (Long) geneManager.getSql().getObject("select max(numappr)+1 from PTAPPROVAZIONI where conint = ? ", new Object[] { newConintAppr });
