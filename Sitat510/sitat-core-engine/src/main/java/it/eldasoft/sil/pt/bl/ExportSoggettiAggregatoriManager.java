	...
	private void fillSchedaB(HSSFSheet sheet, int indiceRigaCorrente, List<?> dati,
      ...
      if (j==18) {
        ...
        // campi di tipo importo
	      Double importo = (Double) jdbcParam.getValue();
	      if (importo != null) {
	        UtilityExcel.scriviCella(sheet, (j+1), indiceRigaCorrente, importo, dizStiliExcel.getStileExcel(DizionarioStiliExcel.DECIMALE2_ALIGN_RIGHT));
	      } else {
	        UtilityExcel.scriviCella(sheet, (j+1), indiceRigaCorrente, new Double(0), 
