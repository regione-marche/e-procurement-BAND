...
<%-- 							</gene:campoScheda> --%>
			...
			<gene:campoScheda>
				<c:if test='${ (stato eq 12) || (gene:checkProt(pageContext, "FUNZ.VIS.ALT.W9.W9HOME.PIATRI") && datiRiga.RIS_CAPITOLO_CONTRI eq 0) }' >
		...
		<gene:javaScript>
			...
			$('$[id^="RIS_CAPITOLO_DV"]').change(
				function() {
					calcoloImportiTotaliQR("DV")
				}
			);
			
			$('$[id^="RIS_CAPITOLO_MU"]').change(
				function() {
					calcoloImportiTotaliQR("MU")
				}
			);

			$('$[id^="RIS_CAPITOLO_BI"]').change(
				function() {
					calcoloImportiTotaliQR("BI")
				}
			);

			$('$[id^="RIS_CAPITOLO_AP"]').change(
				function() {
					calcoloImportiTotaliQR("AP")
				}
			);

			$('$[id^="RIS_CAPITOLO_AL"]').change(
				function() {
					calcoloImportiTotaliQR("AL")
				}
			);

			$('$[id^="RIS_CAPITOLO_IV"]').change(
				function() {
	...
	<c:choose>
		<c:when test='${tiprog eq 2}'>
			...
			$('#scf_3a').hide();
			<c:choose>
				<c:when test='${ModificheRER_RIS_CAPITOLO}'>
					$('#scf_3a').show();
				</c:when>
				<c:otherwise>
					$('#scf_3a').hide();					
