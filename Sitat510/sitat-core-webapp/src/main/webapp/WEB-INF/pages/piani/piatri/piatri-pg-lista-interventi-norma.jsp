	...
	<td>
	  <gene:formLista entita="INTTRI" pagesize="20" sortColumn="3;4"
		...
		<c:set var="link"
			...
			<c:if test='${true or (sequenzaInterventiOk eq "TRUE")}'>
				<c:choose>
					<c:when test='${true or (updateLista eq "1")}' >
						<gene:campoLista campo="NPROGINT" title="Nr." width="30" href="${gene:if(visualizzaLink, link, '')}" ordinabile="false" />
					</c:when>
					<c:otherwise>
						<gene:campoLista campo="NPROGINT" title="Nr." width="50" href="${gene:if(visualizzaLink, link, '')}" ordinabile="false" >
			...
			<gene:campoLista campo="DESINT" width="390" ordinabile="false" href="${gene:if(visualizzaLink, link, '')}" />
...
<gene:javaScript>
	...
	function approvaFabbisogniListaSelezionati(funzionalita) {
		...
		/////////////////
		location.href = "${pageContext.request.contextPath}/ApriPagina.do?href=piani/ptapprovazioni/ptapprovazioni-scheda.jsp&modo=NUOVO&fabbisogni="+valoriFabbisogni+"&funzionalita="+funzionalita+"&contri_PT=${contri}&conint_PT="+valoriConintPT;
	}
	
	function approvaFabbisognoListaPopUpItem(chiave,funzionalita,valoriConintPT) {
		location.href = "${pageContext.request.contextPath}/ApriPagina.do?href=piani/ptapprovazioni/ptapprovazioni-scheda.jsp&modo=NUOVO&fabbisogni="+chiave+"&funzionalita="+funzionalita+"&contri_PT=${contri}&conint_PT="+valoriConintPT;	
	}
