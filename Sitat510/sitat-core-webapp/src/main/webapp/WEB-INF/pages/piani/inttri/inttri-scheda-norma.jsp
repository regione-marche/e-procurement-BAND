	...
	<gene:formScheda entita="INTTRI" gestisciProtezioni="true" gestore="it.eldasoft.sil.pt.tags.gestori.submit.GestoreINTTRI"
		...
		<gene:gruppoCampi idProtezioni="GEN">
			...
			<gene:campoScheda campo="CUIINT" modificabile="false" speciale="true" obbligatorio="false" >
				<c:if test='${modoAperturaScheda eq "MODIFICA" or modo eq "MODIFICA"}' >
		...
		<c:choose>
			...
			<gene:campoScheda campo="CODCPV" visibile="${tiprog eq 2}" href="#" speciale="true" >
				<gene:popupCampo titolo="Dettaglio CPV" href="formNUTS(${modCODNUTS},'W9LOTT_LUOGO_NUTS')" />
			</gene:campoScheda>
			...
			<gene:campoScheda campo="INTERV" visibile="${tiprog ne 2}" />
					...
					<td colspan="2">
						...
						<table class="griglia">
							...
							<tr>
								...
								<gene:campoScheda campo="PR3TRI" addTr="false" hideTitle="true" visibile="${datiRiga.PIATRI_TIPROG eq '1'}" />
								<gene:campoScheda campo="PR9TRI" addTr="false" hideTitle="true" />
...
<%-- 								<gene:campoScheda campo="IV3TRI" campoFittizio="true" definizione="F24.5;0;;MONEY;" visibile="${datiRiga.PIATRI_TIPROG eq '1'}" modificabile="false" addTr="false" hideTitle="true"/> --%>
	...
	<c:choose>
		<c:when test='${tiprog eq 2 and gene:checkProt(pageContext, "COLS.VIS.PIANI.INTTRI.IV1TRI")}'>
			$('#di_cui_iva').show();
			$('#imp_netto_iva').show();
			$('#scf_3a').hide();
		</c:when>
