...
<gene:template file="popup-template.jsp" gestisciProtezioni="true" schema="PIANI" idMaschera="listaInterventi">
	...
	<c:choose>
		...
		<c:otherwise>
				<c:choose>
					<c:when test='${tiprog eq 1}'>
						La lista presenta l'elenco degli interventi dei programmi dell'anno precedente che non sono gi&agrave; stati riportati nel programma attuale
					</c:when>
					<c:otherwise>
						La lista presenta l'elenco degli acquisti dei programmi dell'anno precedente che non sono gi&agrave; stati riportati nel programma attuale
					</c:otherwise>
				</c:choose>
		</c:otherwise>
	</c:choose>
...
<c:choose>
	...
	<gene:javaScript>
		...
		function richiediRevisione() {
			...
			location.href = "${pageContext.request.contextPath}/ApriPagina.do?href=piani/ptapprovazioni/ptapprovazioni-popup-scheda.jsp&modo=NUOVO&fabbisogni="+valoriFabbisogni+"&funzionalita=${param.tipo}&contri_PT=${contri}";
		}
		...
		function filtraLista() {
			...
			var temp = "GetListaInterventiDaProgrammaPrecedente.do?codiceProgramma=${param.codiceProgramma}&tipo=${param.tipo}&numeroPopUp=1";
