...
<gene:formScheda entita="INTTRI" gestisciProtezioni="true" gestore="it.eldasoft.sil.pt.tags.gestori.submit.GestoreFABBISOGNI"
	...
	<c:set var="isModifcabileAnnoeQuadro" value="${datiRiga.FABBISOGNI_STATO ne '12'}" />
...
<!-- 		----------------------------- -->
		...
		<c:if test='${modoAperturaScheda ne "VISUALIZZA"}'>
			...
			<c:if test='${!empty listaValoriAILINT}'>
				<c:forEach items="${listaValoriAILINT}" var="valoriAILINT">
...
<!-- 		-----------------------------		 -->
		...
		<gene:campoScheda campo="NUTS" href="javascript:formNUTS(${modCODNUTS},'INTTRI_NUTS')" modificabile="false" speciale="true" >
			<gene:popupCampo titolo="Dettaglio codice NUTS" href="formNUTS(${modCODNUTS},'INTTRI_NUTS')" />
		</gene:campoScheda>
		
				
<%-- 		<gene:campoScheda campo="CODCPV"/> --%>
		<gene:campoScheda campo="CODCPV" href='javascript:formCPV(${modoAperturaScheda ne "VISUALIZZA"})' modificabile="false" speciale="true" visibile="${tipint eq 2}" >
				<c:if test='${modoAperturaScheda ne "VISUALIZZA"}'>
		...
		<gene:campoScheda campo="CPVDESC" title="Descrizione CPV" value="${cpvdescr}"	href='javascript:formCPV(${modoAperturaScheda ne "VISUALIZZA"})'
			campoFittizio="true" modificabile="false" definizione="T150" visibile="${tipint eq 2}" />
...
<%-- 								</gene:campoScheda> --%>
			...
			<gene:gruppoCampi idProtezioni="ACQVERDI">
				...
				<gene:campoScheda campo="AVCPV" href="javascript:formCPV1(${isModifica}, 'INTTRI_AVCPV', 'AVCPVDESC')" modificabile="false" speciale="true">
					<c:if test='${modoAperturaScheda ne "VISUALIZZA"}'>
						<gene:popupCampo titolo="Dettaglio codice CODCPV" href="javascript:formCPV1(${isModifica}, 'INTTRI_AVCPV', 'AVCPVDESC')" />
					</c:if>
				</gene:campoScheda>
				<gene:campoScheda campo="AVCPVDESC" title="Descrizione CPV" value="${avcpvdescr}" href="javascript:formCPV1(${isModifica}, 'INTTRI_AVCPV', 'AVCPVDESC')"
			...
			<gene:gruppoCampi idProtezioni="MATRIC">
				...
				<gene:campoScheda campo="MRCPV"  href="javascript:formCPV1(${isModifica}, 'INTTRI_MRCPV', 'MRCPVDESC')" modificabile="false" speciale="true">
					<c:if test='${modoAperturaScheda ne "VISUALIZZA"}'>
						<gene:popupCampo titolo="Dettaglio codice CODCPV" href="javascript:formCPV1(${isModifica}, 'INTTRI_MRCPV', 'MRCPVDESC')" />
					</c:if>
				</gene:campoScheda>
				<gene:campoScheda campo="MRCPVDESC"  title="Descrizione CPV" value="${mrcpvdescr}"	href="javascript:formCPV1(${isModifica}, 'INTTRI_MRCPV', 'MRCPVDESC')"
					campoFittizio="true" modificabile="false" definizione="T150" />
			...
			<gene:gruppoCampi idProtezioni="ALTDAT">
				...
				<c:choose>
					<c:when test='${tipint eq 1}'>
						...
						value="${datiRiga.INTTRI_VARIATO}" visibile="false"/>
					</c:when>
					<c:when test='${tipint eq 2}'>
	    		...
	    		<c:when test='${isNavigazioneDisabilitata ne "1"}'>
	    			<c:if test="${datiRiga.FABBISOGNI_STATO eq '1' || datiRiga.FABBISOGNI_STATO eq '2' || datiRiga.FABBISOGNI_STATO eq '11'}">
	    				<c:if test="${checkProtRDS_SottoponiAdApprFinanz}">
		      			<td class="vocemenulaterale">
									<a href="javascript:eseguiFabbisognoScheda('${key}','RDS_SottoponiAdApprFinanz');" title="Sottoponi proposta ad approvazione finanziaria" tabindex="1520">
										Sottoponi proposta ad approvazione finanziaria
									</a>
				  			</td>
		  				</c:if>
		  			</c:if>
...
<%-- 	    			<c:if test="${datiRiga.FABBISOGNI_STATO eq '4' || datiRiga.FABBISOGNI_STATO eq '21'}"> --%>
	    		...
	    		<c:when test='${isNavigazioneDisabilitata ne "1"}'>
	    			<c:if test="${datiRiga.FABBISOGNI_STATO eq '4' || datiRiga.FABBISOGNI_STATO eq '21'}">
	    				<c:if test="${checkProtRDP_SegnalaProceduraAvviata}">
		      			<td class="vocemenulaterale">
									<a href="javascript:eseguiFabbisognoScheda('${key}','RDP_SegnalaProceduraAvviata');" title="Segnala Procedura Avviata" tabindex="1533">
										Segnala Procedura Avviata
									</a>
				  			</td>
			  			</c:if>
			  		</c:if>
	...
	<gene:javaScript>
		...
		$(document).ready(function() {
			...
			$('#INTTRI_PR1TRI').change(function() {
				checkTipCapPriv();
			});
			$('#INTTRI_PR2TRI').change(function() {
				checkTipCapPriv();
			});
			$('#INTTRI_PR3TRI').change(function() {
				checkTipCapPriv();
			});
			$('#INTTRI_PR9TRI').change(function() {
				checkTipCapPriv();
			});
			...
			$('#INTTRI_ACQALTINT').change(function() {
				acqAltInt = toVal(getValue('INTTRI_ACQALTINT'));
				if (acqAltInt == 2) {
					$("#rowINTTRI_CUICOLL").show();
				} else {
	...
	<c:choose>
		...
		$('$[id^="IMMTRAI_VA1IMM_"]').change(
			function() {
				calcoloImportiTotaliImmobili(1)
			}
		);
		
		$('$[id^="IMMTRAI_VA2IMM_"]').change(
			function() {
				calcoloImportiTotaliImmobili(2)
			}
		);
		$('$[id^="IMMTRAI_VA3IMM_"]').change(
			function() {
				calcoloImportiTotaliImmobili(3)
			}
		);
		$('$[id^="IMMTRAI_VA9IMM_"]').change(
			function() {
	...
	<c:choose>
		<c:when test='${tipint eq 2 and gene:checkProt(pageContext, "COLS.VIS.PIANI.INTTRI.IV1TRI")}'>
			$('#di_cui_iva').show();
			$('#imp_netto_iva').show();
			<c:choose>
				<c:when test='${ModificheRER}'>
					$('#scf_3a').show();
				</c:when>
				<c:otherwise>
					$('#scf_3a').hide();					
