...
<gene:template file="popup-template.jsp">
	...
	<c:choose>
		...
		<c:when test='${funzionalita eq "RDS_SottoponiAdApprFinanz"}'>
			<gene:setString name="titoloMaschera"  value='Sottoponi Proposte al RAF' />
			<c:set var="descr1" value="Il controllo dei dati delle proposte non ha rilevato situazioni anomale:" />
			<c:set var="descr2" value="E' possibile inoltrarli al RAF" />			
			<c:set var="codFunzione" value="INOLTRO_RAF" />
		</c:when>
		...
		<c:when test='${funzionalita eq "RDP_SegnalaProceduraAvviata"}'>
			<gene:setString name="titoloMaschera"  value='Segnala Procedura Avviata' />
			<c:set var="descr1" value="Confermare che la procedura di acquisto è stata avviata? L'operazione non potrà essere annullata" />	
			<c:set var="descr2" value="" />		
		</c:when>
		<c:otherwise>
