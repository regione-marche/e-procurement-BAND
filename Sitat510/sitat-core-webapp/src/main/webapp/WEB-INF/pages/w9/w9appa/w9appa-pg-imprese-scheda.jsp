...
<gene:template file="scheda-template.jsp" gestisciProtezioni="true" schema="W9" idMaschera="W9APPA-scheda">
	...
	<gene:redefineInsert name="corpo">
		...
		<gene:formScheda entita="W9APPA" gestisciProtezioni="true"
			...
			<c:choose>
				<c:when test='${numImpr ne null}'>
					<c:set var="addSelectNumImpr"
						value=" and W9AGGI.NUM_AGGI = ${numImpr} " />
			...
			<gene:campoScheda entita="W9AGGI" campo="CODGARA"
				where="W9AGGI.CODGARA = ${codgara} and W9AGGI.CODLOTT = ${codlott} and W9AGGI.NUM_APPA = ${num} ${addSelectNumImpr}"
			...
			<jsp:include
				...
				<jsp:param name="arrayCampi"
					value="'W9AGGI_CODGARA_','W9AGGI_CODLOTT_','W9AGGI_NUM_APPA_','W9AGGI_NUM_AGGI_','IMPR_NOMEST_','W9AGGI_CODIMP_','W9AGGI_ID_TIPOAGG_','W9AGGI_RUOLO_','W9AGGI_FLAG_AVVALIMENTO_','IMPR_NOMEST_AUSILIARIA_','W9AGGI_CODIMP_AUSILIARIA_','W9AGGI_ID_GRUPPO_'" />
				<jsp:param name="arrayVisibilitaCampi"
					value="false,false,false,false,true,false,false,true,true,false,false,false" />
	...
	<gene:javaScript>
		...
		<c:if test='${numImpr ne null}'>
			function schedaModifica(){
				...
				if ("" == idGruppo) {
					...
					var href = "href=w9/w9appa/w9appa-pg-imprese-scheda.jsp&modo=MODIFICA&operazioneModificaAggi=1&isW9FasePartecipante=" + isW9FasePartecipante + "&numImpr=" + numImpresa + "&key=" + tempKey;
					document.location.href = "${pageContext.request.contextPath}/ApriPagina.do?" + href;
				} else {
					// Modifica dettaglio di un gruppo di imprese
					var href = "href=w9/w9appa/w9appa-pg-imprese-scheda.jsp&modo=MODIFICA&operazioneModificaAggi=1&isW9FasePartecipante=" + isW9FasePartecipante + "&numImpr=" + numImpresa + "&id_gruppo=" + idGruppo + "&key=" + tempKey;
					document.location.href = "${pageContext.request.contextPath}/ApriPagina.do?" + href;
				}
			}
		</c:if>
		...
		showNextElementoSchedaMultipla = function(tipo, campi, visibilitaCampi) {
			...
			if (tipoAggiudicataria == 1){
				tmp(tipo, campi, new Array(false,false,false,false,true,false,false,true,true,false,false,false));
			}
			else {
				tmp(tipo, campi, new Array(false,false,false,false,true,false,false,false,true,false,false,false));
			}
		}
	...
	<c:if test='${modo eq "MODIFICA"}' >
		<c:set var="localkey" value='${codgara};${codlott};${datiRiga.W9FASI_FASE_ESECUZIONE};${datiRiga.W9FASI_NUM}' scope="request" />
