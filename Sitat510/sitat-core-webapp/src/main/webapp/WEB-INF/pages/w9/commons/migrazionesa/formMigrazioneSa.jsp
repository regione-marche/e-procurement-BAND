...
<form method="post" name="formMigrazioneSA" action="${pageContext.request.contextPath}/w9/MigraSA.do" >
			...
			<td class="valore-dato" colspan="3">
				<c:choose>
					<c:when test="${sessionScope.MIGRAZIONE_SA_BEAN.numeroGareDaMigrare eq 1}">
						<br>
						La gara sar&agrave; migrata alla stazione appaltante ${sessionScope.MIGRAZIONE_SA_BEAN.nomeStazAppDestinazione} (C.F. ${sessionScope.MIGRAZIONE_SA_BEAN.codiceFiscaleStazAppDestinazione}). Continuare?
						<br><br>
					</c:when>
					<c:otherwise>
						<br>
						Le gare saranno migrate alla stazione appaltante ${sessionScope.MIGRAZIONE_SA_BEAN.nomeStazAppDestinazione} (C.F. ${sessionScope.MIGRAZIONE_SA_BEAN.codiceFiscaleStazAppDestinazione}). Continuare?
						<br><br>
					</c:otherwise>
				</c:choose>
			</td>
