...
<gene:redefineInsert name="addToAzioni">
	<c:choose>
		...
		<c:when test='${modo eq "VISUALIZZA"}'>
			<c:if test="${(userIsRup eq 'si' || permessoUser ge 4 || sessionScope.profiloUtente.abilitazioneStd eq 'A')}">
				<tr>
					<td class="vocemenulaterale">
...
<gene:redefineInsert name="addToDocumenti">
	<c:if test='${modo eq "VISUALIZZA" and isAttivoIndicatoriLotto eq true}'>
		<tr>
			<td class="vocemenulaterale">
	...
	<gene:gruppoCampi idProtezioni="GEN">
		...
		<gene:campoScheda campo="COMCON" />
		<gene:campoScheda campo="DESCOM" />
		<gene:campoScheda campo="CIGCOM" />
...
<c:choose>
	...
	<c:otherwise>
		...
		<gene:campoScheda campo="ART_E1" visibile="${datiRiga.W9GARA_VER_SIMOG eq 1}" >
			<gene:checkCampoScheda
...
<c:choose>
	...
	<c:otherwise>
		...
		<gene:campoScheda campo="CPV" title="Codice CPV" href="#" speciale="true" >
	...
	<gene:gruppoCampi idProtezioni="CPVSEC">
		...
		<jsp:include page="/WEB-INF/pages/commons/interno-scheda-multipla.jsp" >
			...
			<jsp:param name="arrayCampi" value="'W9CPV_CODGARA','W9CPV_CODLOTT', 'W9CPV_NUM_CPV_', 'W9CPV_CPV_','TABCPV_CPVDESC_'" />
...
<c:choose>
	...
	<gene:gruppoCampi idProtezioni="ULTCAT">
		...
		<gene:campoScheda campo="LUOGO_NUTS" href="javascript:formNUTS(${modCODNUTS},'W9LOTT_LUOGO_NUTS')" modificabile="false" speciale="true" >
			<gene:popupCampo titolo="Dettaglio codice NUTS" href="formNUTS(${modCODNUTS},'W9LOTT_LUOGO_NUTS')" />
		</gene:campoScheda>
...
<gene:javaScript>
	...
	function bloccaCampo(valore, campo) {
		if (campo==3) {
			var vis = (valore==1);
			showObj("rowW9LOTT_DESCOM",vis);
			showObj("rowW9LOTT_CIGCOM",vis);
			if (!vis) {
				setValue("W9LOTT_DESCOM","");
				setValue("W9LOTT_CIGCOM","");
			}
		} else if (campo==1) {
	...
	function riallineaSIMOG() {
		...
		openPopUpCustom('href=w9/w9lott/riallineaSimogPopup.jsp&codGara='+codGara+'&codiceCIG='+codiceCIG, 'formRiallineaSIMOG', 545, 280, 1, 1);
	}
...
<c:if test="${modo ne 'NUOVO'}" >
	...
	function sceltaTipoReportIndicatori() {
		openPopUpCustom("href=w9/w9lott/popup-sceltaTipoReportIndicatori.jsp", "indicatori", 600, 225, "yes", "yes");
	}
