...
<c:otherwise>
	...
	<gene:javaScript>
		...
		<c:if test='${msgEsito eq "ko"}'>
			<c:choose>
				...
				<c:otherwise>
					alert("Invio non riuscito. Si sono verificati degli errori durante l'invio dei dati a Simog");
				</c:otherwise>
			</c:choose>
		</c:if>
	...
	<c:choose>
		...
		function scegliFase () {
			...
			if (chosen != '') {
				var action="${pageContext.request.contextPath}/ApriPagina.do?href=w9/w9flussi/w9flussi-scheda.jsp";
