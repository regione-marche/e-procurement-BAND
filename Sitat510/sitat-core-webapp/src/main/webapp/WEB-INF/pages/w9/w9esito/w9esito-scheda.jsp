...
<gene:template file="scheda-template.jsp" gestisciProtezioni="true" schema="W9" idMaschera="W9ESITO-A22-scheda">
	...
	<gene:setString name="titoloMaschera" value='${gene:if("NUOVO"==modo, "Nuova Fase di comunicazione esito", "Fase di comunicazione esito")}' />	
	...
	<gene:redefineInsert name="corpo">
		...
		<gene:formScheda entita="W9ESITO" gestisciProtezioni="true"
			...
			<gene:campoScheda campo="ESITO_PROCEDURA" defaultValue="${requestScope.esito}" modificabile='${esito ne 1 or esitiUguali eq "true"}' />
			<gene:campoScheda campo="IMPORTO_AGGIUDICAZIONE" defaultValue="${requestScope.importoAggiu}" />
			<c:set var="labelData" value="Data aggiudicazione definitiva" />
			<c:if test='${esito > 1}'>
				<c:set var="labelData" value="Data verbale" />
			</c:if>
			<gene:campoScheda campo="DATA_VERB_AGGIUDICAZIONE" defaultValue="${requestScope.dataVerbAggiu}" title="${labelData}">
			...
			<gene:campoScheda campo="VISUALIZZAFILEALLEGATO"
				title="File PDF per la pubblicazione sul sito della Regione"
			...
			<gene:campoScheda campo="CODGARA" entita="W9FASI" visibile="false"  value="${codgara}"
				where="W9FASI.CODGARA=W9ESITO.CODGARA AND W9FASI.CODLOTT=W9ESITO.CODLOTT AND W9FASI.FASE_ESECUZIONE=984" />
	...
	<gene:javaScript>
		...
		function apriAllegato() {
			...
			document.location.href=actionAllegato+"?"+par;
		}
