...
<gene:javaScript>
	...
	function aggiungi() {
		document.location.href ="${pageContext.request.contextPath}/ApriPagina.do?href=w9/w9pubblicazioni/w9pubblicazioni-scheda.jsp&key=W9PUBBLICAZIONI.CODGARA=N:${codiceGara}&modo=NUOVO&tipoPubblicazione=${tipoPubblicazione}&codiceLotto=${codiceLotto}";
	}
	
	function elimina() {
		if(confirm("Eliminare l'atto e documenti corrispondenti?")) {
			// vedi w9flussi logica
			document.location.href ="${pageContext.request.contextPath}/w9/CancellaPubblicazione.do?codgara=${codiceGara}&numpubb=${numeroPubblicazione}";
		}
	}
...
<c:if test="${modo eq 'MODIFICA' || modo eq 'NUOVO'}">
	...
	function visualizzaFileAllegato(numeroDoc) {
		...
		document.location.href = href+"?codgara=${codiceGara}&numPubb=${numeroPubblicazione}&numdoc=" + numeroDoc + "&tab=gara";
	}
