...
<gene:formScheda entita="W3LOTT" gestisciProtezioni="true" 
	...
	<c:choose>
		<c:when test='${not empty modo and modo ne "VISUALIZZA" }'>
			<c:set var="modNUTS" value="true" scope="request"/>
		</c:when>
		<c:otherwise>
			<c:set var="modNUTS" value="false" scope="request"/>
		</c:otherwise>
	</c:choose>
	<gene:campoScheda campo="LUOGO_NUTS" href="javascript:formNUTS(${requestScope.modNUTS}, 'W3LOTT_LUOGO_NUTS')" modificabile="false" speciale="true" >
		<gene:popupCampo titolo="Dettaglio codice NUTS" href="formNUTS(${requestScope.modNUTS}, 'W3LOTT_LUOGO_NUTS')" />
	</gene:campoScheda>
	...
	<gene:campoScheda campo="FLAG_PREVEDE_RIP" defaultValue="2" obbligatorio="true"/>
...
<gene:javaScript>
	...
	function formNUTS(modifica, campo){
			openPopUpCustom("href=w3/commons/dettaglio-codice-nuts.jsp&key=" + document.forms[0].key.value + "&keyParent=" + document.forms[0].keyParent.value + "&modo="+(modifica ? "MODIFICA":"VISUALIZZA")+"&campo="+campo+"&valore="+ getValue(campo), "formNUTS", 700, 300, 1, 1);
	}
