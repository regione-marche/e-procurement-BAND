...
<%@ page language="java" import="it.avlp.simog.common.beans.ResponseCancellaGara" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponseCancellaLotto" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponseCheckLogin" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponseChiudiSession" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponseConsultaGara" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponseInserisciGara" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponseInserisciLotto" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponsePerfezionaGara" %>
<%@ page language="java" import="it.avlp.simog.common.beans.ResponsePerfezionaLotto" %>
<%@ page language="java" import="it.avlp.simog.ws.beans.SimogWS" %>
<%@ page language="java" import="it.avlp.simog.ws.beans.SimogWSServiceLocator" %>
	...
	<body>
	    ...
	    try {
			  ...
			  if (StringUtils.isEmpty(password)) {
				...
				it.avlp.simog.massload.xmlbeans.SimogWSPDDServiceStub simogWsPddServiceStub = null;

				// Creazione della options contenente i parametri necessari alla basic authentication
				org.apache.axis2.transport.http.HttpTransportProperties.Authenticator basicAuth = null;

				String usernameBasicAuthPDD = ConfigManager.getValore(CostantiSimog.PROP_PDD_BA_SIMOGWS_USERNAME);
				String passwordBasicAuthPDD = ConfigManager.getValore(CostantiSimog.PROP_PDD_BA_SIMOGWS_PASSWORD);

				if (StringUtils.isNotEmpty(passwordBasicAuthPDD)) {
					ICriptazioneByte criptazione = FactoryCriptazioneByte.getInstance(
						ConfigManager.getValore(CostantiGenerali.PROP_TIPOLOGIA_CIFRATURA_DATI),
						passwordBasicAuthPDD.getBytes(), ICriptazioneByte.FORMATO_DATO_CIFRATO);
						passwordBasicAuthPDD = new String(criptazione.getDatoNonCifrato());
				}
				
				if (StringUtils.isEmpty(usernameBasicAuthPDD)) {
					listaMessaggi.add("error: La login per la basic authentication alla PDD dei Servizi Simog non e' definita");
				}
					
				if (StringUtils.isEmpty(passwordBasicAuthPDD)) {
				  listaMessaggi.add("error: La password per la basic authentication alla PDD dei Servizi Simog non e' definita");
				}
				...
				if (StringUtils.isNotEmpty(usernameBasicAuthPDD) && StringUtils.isNotEmpty(passwordBasicAuthPDD)) {
				  ...
				  basicAuth.setUsername(usernameBasicAuthPDD);
				  basicAuth.setPassword(passwordBasicAuthPDD);
				...
				if (basicAuth == null) {
				  ...
				  simogWsPddServiceStub._getServiceClient().getOptions().setProperty(
					  org.apache.axis2.transport.http.HTTPConstants.AUTHENTICATE, basicAuth);
				}
				...
				if (loginResponse.isSetReturn()) {
					...
					if (responseCheckLogin.getSuccess()) {
						...
						if (basicAuth != null) {
							simogWsPddServiceStub = new SimogWSPDDServiceStub(url + "/consultaGara");
							simogWsPddServiceStub._getServiceClient().getOptions().setProperty(
							org.apache.axis2.transport.http.HTTPConstants.AUTHENTICATE, basicAuth);
						}
						...
						simogWsPddServiceStub = new SimogWSPDDServiceStub(url + "/chiudiSessione");
						if (basicAuth != null) {
							simogWsPddServiceStub = new SimogWSPDDServiceStub(url + "/chiudiSessione");
							simogWsPddServiceStub._getServiceClient().getOptions().setProperty(
							org.apache.axis2.transport.http.HTTPConstants.AUTHENTICATE, basicAuth);
			...
			listaMessaggi.add("error: Errore nella decriptazione di username o password per la Basic Authentication "
				+ "alla PDD. Contattare un amministratore.");
			log("error: Errore nella decriptazione di username o password per la Basic Authentication "
