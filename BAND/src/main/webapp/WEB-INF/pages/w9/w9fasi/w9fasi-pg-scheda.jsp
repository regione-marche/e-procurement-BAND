...
<gene:redefineInsert name="addToAzioni">
	<tr>
		<td class="vocemenulaterale"><a href="javascript:scegliFase('${key}');"
...
<form id="form1" name="form1">
	<table class="dettaglio-tab" >
		<tr><td colSpan="2" ><b>Fasi attraverso le quali si sviluppa l'attività di rilevazione dei dati</b></td></tr>
		<c:forEach begin="0" end="${fn:length(datiTabellatoList)-1}" var="indiceTabCond">
			<c:if test='${datiTabellatoList[indiceTabCond][2] < 100}'>
		...
		<c:forEach begin="0" end="${fn:length(datiTabellatoList)-1}" var="indiceTabCond100">
			<c:if test='${datiTabellatoList[indiceTabCond100][2] >= 100}'>
	...
	<gene:javaScript>
		function scegliFase (chiave) {
			...
			if (chosen != '') {
				location.href = '${pageContext.request.contextPath}/w9/RedirectNuovaFase.do?chosen=' + chosen + '&keyatt=' + chiave;
			} else {
				alert('Per procedere è necessario selezionare una fase.');
			}
		}
		
		
		var arrayRadioB = document.getElementsByName("group1");
		
		if (arrayRadioB.length > 0) {
			for (var ii=0; ii < arrayRadioB.length; ii++) {
				if (!arrayRadioB[ii].disabled) {
					if (arrayRadioB[ii].value == 984) {
						arrayRadioB[ii].checked = true;
					} else {
						arrayRadioB[ii].disabled = true;
					}
				}
			}
		}
