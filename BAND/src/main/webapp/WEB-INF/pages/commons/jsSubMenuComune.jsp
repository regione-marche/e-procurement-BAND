...
<!-- 
	...
	<c:if test='${gene:checkProt(pageContext, "SUBMENU.VIS.ARCHIVI.Archivio-tecnici-imprese")}'>
	 ...
	 <c:if test='${gene:checkProt(pageContext, "SUBMENU.VIS.ARCHIVI.Archivio-utenti")}'>
		linksetSubMenuArchivi += creaVoceSubmenu("${contextPath}/ApriPagina.do?href=gene/utent/utent-trova.jsp", 1244, "Archivio utenti");
	</c:if>
  ...
  <c:if test='${(profiloUtente.utenteLdap == 0 or profiloUtente.utenteLdap == 3) and (empty sessionScope.accountCohesion) and gene:checkProt(pageContext,"SUBMENU.VIS.UTILITA.Cambia-password")}'>
  ...
  var linksetSubMenuAmministrazione = "";
  <c:if test='${fn:contains(listaOpzioniUtenteAbilitate, "ou89#") and gene:checkProt(pageContext,"SUBMENU.VIS.AMMINISTRAZIONE.Configurazione-MAIL")}'>
  	linksetSubMenuAmministrazione += creaVoceSubmenu("${contextPath}/ConfigurazioneMail.do?metodo=visualizza", 1281, "Configurazione server di posta");
</c:if>
  <c:if test='${fn:contains(listaOpzioniDisponibili, "OP100#") && fn:contains(listaOpzioniUtenteAbilitate, "ou89#") and gene:checkProt(pageContext,"SUBMENU.VIS.AMMINISTRAZIONE.Configurazione-LDAP")}'>
  	linksetSubMenuAmministrazione += creaVoceSubmenu("${contextPath}/ConfigurazioneLdap.do?metodo=visualizza", 1282, "Configurazione LDAP");
  </c:if>
